...
export function getSafeDecisionMessage(decisionCode) {
  switch (decisionCode) {
    case 'blocked_missing_employee_id':
    case 'blocked_invalid_employee_id':
      return { name: 'Employee ID needed', detail: 'Enter your employee ID before scanning.' }
    case 'blocked_unknown_employee_id':
      return { name: 'ID not found', detail: 'Check your employee ID or complete registration first.' }
    case 'blocked_employee_id_conflict':
      return { name: 'ID conflict', detail: 'This employee ID matches multiple records. Contact HR.' }
    case 'blocked_claimed_employee_mismatch':
      return { name: 'ID mismatch', detail: 'Your face does not match the entered employee ID.' }
    case 'blocked_no_biometrics':
    case 'blocked_biometrics_inactive':
      return { name: 'Re-enroll needed', detail: 'Your employee biometric profile needs HR review or re-enrollment.' }
    ...
    case 'blocked_missing_gps':
      return { name: 'Location needed', detail: 'Enable location services before scanning.' }
    case 'blocked_geofence':
      return { name: 'Outside office', detail: 'You are outside the office geofence area.' }
    case 'blocked_capture_policy':
      return { name: 'Capture too weak', detail: 'Hold the phone upright, keep your face centered, and try again.' }
    case 'blocked_descriptor_shape':
      return { name: 'Capture invalid', detail: 'Face capture was not valid. Hold steady and try again.' }
    ...
    case 'blocked_workforce_policy_unavailable':
      return { name: 'Schedule unavailable', detail: 'Attendance schedule is temporarily unavailable. Contact HR or an administrator.' }
    ...
    case 'blocked_antispoof':
      return { name: 'Photo detected', detail: 'Photo or screen detected.' }
    ...
    case 'blocked_invalid_challenge':
      return { name: 'Session expired', detail: 'Scan session challenge failed. Retry the scan.' }
    case 'blocked_expired_challenge':
      return { name: 'Session expired', detail: 'Scan session challenge expired. Retry the scan.' }
