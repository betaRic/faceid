...
export function getSafeDecisionMessage(decisionCode) {
  switch (decisionCode) {
    ...
    case 'blocked_ambiguous_match':
      return { name: 'Ambiguous match', detail: 'Face matches multiple employees. Contact HR to verify your enrollment.' }
    ...
    case 'blocked_liveness_failed':
      return { name: 'Hold still check', detail: 'Move your head slightly left or right and try again.' }
case 'blocked_index_building':
      return { name: 'Index building', detail: 'System is syncing. Try again in a few seconds.' }
    default:
      return { name: 'Try again', detail: `Unable to process. Try again. Code: ${decisionCode || 'unknown'}` }
  }
}
