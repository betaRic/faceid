...
import { sanitizeLocationReport } from '../location-report-policy.js'
import { buildLocationEvidence } from '../maintenance/location-evidence.js'
  ...
  try {
    return await withPostgresTransaction(async client => {
      ...
      const result = await client.query(`SELECT data, received_at, count(*) OVER () AS total
        ...
        AND COALESCE(data->>'stage', '') <> 'location'
...
export async function loadLocationReports(window, transaction = withPostgresTransaction) {
  try {
    return await transaction(async client => {
      await client.query("SET LOCAL statement_timeout = '1500ms'")
      const result = await client.query(`SELECT data, received_at, count(*) OVER () AS total
        FROM phone_scan_reports WHERE data->>'stage' = 'location'
        AND received_at >= to_timestamp($1 / 1000.0) AND received_at < to_timestamp($2 / 1000.0)
        AND received_at > now() - interval '14 days'
        ORDER BY received_at DESC, id DESC LIMIT 10000`, [window.startMs, window.endMs])
      const reports = result.rows.map(row => {
        const safe = sanitizeLocationReport(row.data)
        return safe ? { ...safe, receivedAt: new Date(row.received_at).toISOString() } : null
      }).filter(Boolean)
      const total = Number(result.rows[0]?.total || 0), truncated = total > reports.length
      return { available: true, source: 'unverified_location_observations', retentionDays: 14,
        total, examined: reports.length, truncated, ...buildLocationEvidence(reports, { truncated }),
        detailTruncated: reports.length > 500, reports: reports.slice(0, 500) }
    })
  } catch {
    return { available: false, groups: [], reports: [], reason: 'Location reports unavailable.' }
  }
}
