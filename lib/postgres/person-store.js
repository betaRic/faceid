...
export async function loadLocalPersonDirectory(options = {}) {
  ...
  const searchMode = options.searchMode === 'employeeId' ? 'employeeId' : 'name'
  const searchValue = String(options.searchValue || '').trim().toLowerCase()
  const limit = Math.max(1, Math.min(50, Number(options.limit) || 25))
  const cursor = options.cursor && options.cursor.mode === searchMode ? options.cursor : null
  const values = []
  ...
  if (approval !== 'all') filters.push(`approval_status = ${addValue(approval)}`)
...
export async function enrollLocalPerson(body, office, resolvedSession = null) {
  ...
  const transactionResult = await withPostgresTransaction(async client => {
    // Plantilla and COS staff can share an Employee ID. A public enrollment is
    // therefore always a new person; explicit re-enrollment uses personId.
    const existing = null
