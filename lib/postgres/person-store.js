...
export async function enrollLocalPerson(body, office, resolvedSession = null) {
  ...
  const transactionResult = await withPostgresTransaction(async client => {
    ...
    await client.query(
      `
        INSERT INTO persons (
          id, employee_id, employee_id_lower, name, name_lower, last_name, first_name, middle_name, position,
  ...
  return withPostgresTransaction(async client => {
    ...
    if (employeeIdChanged) {
      const duplicate = await client.query(
        'SELECT id FROM persons WHERE employee_id_lower = $1 AND id <> $2 LIMIT 1',
        [employeeIdLower, personId],
      )
      if (duplicate.rows[0]) {
    ...
    await client.query(
      `
        ...
        SET employee_id = $2,
            ...
            approval_status = $15,
            approved_at = $16,
            approval_updated_at = $17,
            approval_updated_by_email = $18,
            duplicate_review_required = $19,
            duplicate_review_status = $20,
            data = data || $21::jsonb,
            updated_at = now()
        WHERE id = $1
      `,
    ...
    if (employeeIdChanged) {
      await client.query('UPDATE attendance SET employee_id = $2, name = $3 WHERE employee_id = $1', [existing.employeeId, nextPerson.employeeId, nextPerson.name])
      await client.query('UPDATE attendance_daily SET employee_id = $2, id = $2 || $3 || date_key, name = $4 WHERE employee_id = $1', [existing.employeeId, nextPerson.employeeId, '_', nextPerson.name])
      await client.query('UPDATE attendance_locks SET employee_id = $2 WHERE employee_id = $1', [existing.employeeId, nextPerson.employeeId])
      await client.query('UPDATE scan_events SET employee_id = $2, name = $3 WHERE employee_id = $1 OR person_id = $4', [existing.employeeId, nextPerson.employeeId, nextPerson.name, personId])
    } else if (existing.name !== nextPerson.name) {
      await client.query('UPDATE attendance SET name = $2 WHERE employee_id = $1', [nextPerson.employeeId, nextPerson.name])
      await client.query('UPDATE attendance_daily SET name = $2 WHERE employee_id = $1', [nextPerson.employeeId, nextPerson.name])
      await client.query('UPDATE scan_events SET name = $2 WHERE employee_id = $1 OR person_id = $3', [nextPerson.employeeId, nextPerson.name, personId])
    }
    await syncLocalPersonBiometricIndex(client, personId, nextPerson)
