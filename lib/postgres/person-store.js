...
import { buildDescriptorBuckets } from '@/lib/biometric-math'
...
import { deduplicateDescriptors, serializeDescriptorSample } from '@/lib/persons/enrollment-descriptors'
...
export async function enrollLocalPerson(body, office, resolvedSession = null) {
  ...
  try {
    ...
    const duplicateCandidates = await loadDuplicateCandidates(client, { includePending: true })
    const duplicateFace = evaluateDuplicateFaceCandidates(duplicateCandidates, body.descriptors, '')
    const exactPendingDuplicate = findExactPendingDuplicateCandidate(
      duplicateCandidates,
      body.descriptors,
      '',
    )
    if (duplicateFace?.duplicate || exactPendingDuplicate) {
      const blockingDuplicate = duplicateFace?.duplicate ? duplicateFace : exactPendingDuplicate
      const dup = blockingDuplicate.person
      throw Object.assign(
        new Error(`Face is too similar to ${dup.name} (${dup.employeeId || 'no employee ID'}). Duplicate enrollment blocked.`),
        {
          duplicateFace: blockingDuplicate,
          ...(exactPendingDuplicate ? { code: 'duplicate_person_registration' } : {}),
        },
      )
    }
    ...
    if (uniqueDescriptors.length < ENROLLMENT_MIN_SAMPLES) {
      throw Object.assign(
    ...
    const now = new Date()
    const nextDescriptors = uniqueDescriptors.map(serializeDescriptorSample)
    const nextPerson = {
      id: personId,
      name: submittedNames.name,
      lastName: submittedNames.lastName,
      firstName: submittedNames.firstName,
      middleName: submittedNames.middleName,
      employeeId: body.employeeId,
      accessCode,
      position: String(body.position || '').trim(),
      nameLower: submittedNames.nameLower,
      officeId: office.id,
      ...
      privacyConsent: body.privacyConsentRecord || null,
      ...
      biometricQualityScore: Number.isFinite(body.captureMetadata?.qualityScore)
        ...
        : null,
      submittedAt: now,
      approvedAt: nextApprovalStatus === PERSON_APPROVAL_APPROVED ? now : null,
    }
    const data = {
