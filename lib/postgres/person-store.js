...
const EXACT_PENDING_DUPLICATE_DISTANCE_MAX = 1e-12
...
export async function enrollLocalPerson(body, office, resolvedSession = null) {
  ...
  try {
    ...
    const exactPendingDuplicate = duplicateFace?.reviewRequired
      && duplicateFace.approvalStatus === PERSON_APPROVAL_PENDING
      && Number.isFinite(duplicateFace.distance)
      && duplicateFace.distance <= EXACT_PENDING_DUPLICATE_DISTANCE_MAX
