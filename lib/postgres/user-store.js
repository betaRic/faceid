...
export async function findLocalStaffByPin(pin) {
  const [adminResult, hrResult] = await Promise.all([
    queryPostgres('SELECT * FROM admin_users WHERE active = true'),
    queryPostgres('SELECT * FROM hr_users WHERE active = true'),
  ])
  return {
    admins: adminResult.rows.map(mapAdmin).filter(user => verifyLocalPin(pin, user.pinHash)),
    hrUsers: hrResult.rows.map(mapHr).filter(user => verifyLocalPin(pin, user.pinHash)),
  }
}
