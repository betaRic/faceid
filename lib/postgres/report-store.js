...
export async function listLocalAttendanceLogs(options = {}) {
  ...
  const result = await queryPostgres(
    `
      ...
      LIMIT $6
    `,
    [employeeId, officeId, dateKey, startMs, endMs, limit],
  )
...
export async function listLocalHrEmployees(options = {}) {
  ...
  const approval = normalizeText(options.approval).toLowerCase()
  const offset = (page - 1) * pageSize

  const params = [officeId, query, status, approval, pageSize, offset]
  const where = `
    WHERE ($1 = '' OR office_id = $1)
      ...
      AND ($4 = '' OR approval_status = $4)
  `

  const [totalResult, rowsResult] = await Promise.all([
    queryPostgres(`SELECT count(*)::integer AS count FROM persons ${where}`, params),
    queryPostgres(
      `
        SELECT *
        FROM persons
...
export async function resetLocalRuntimeData(options = {}) {
  const includeEmployees = options.includeEmployees === true
  const includeOffices = options.includeOffices === true
  return withPostgresTransaction(async client => {
    const counts = {}
    for (const table of ['attendance', 'attendance_daily', 'attendance_locks', 'attendance_challenges', 'scan_events', 'audit_logs', 'biometric_index']) {
      counts[table] = Number((await client.query(`DELETE FROM ${table}`)).rowCount || 0)
    }
    if (includeEmployees) {
      counts.persons = Number((await client.query('DELETE FROM persons')).rowCount || 0)
    } else {
      await client.query(`
        UPDATE persons
    ...
    if (includeOffices) {
      counts.offices = Number((await client.query('DELETE FROM offices')).rowCount || 0)
    }
    return counts
  })
}

export async function resetLocalBiometrics() {
  return withPostgresTransaction(async client => {
    const personsResult = await client.query(`
      ...
      SET descriptors = '[]'::jsonb,
          ...
          updated_at = now()
    `)
    const indexResult = await client.query('DELETE FROM biometric_index')
    return {
      personsUpdated: Number(personsResult.rowCount || 0),
      biometricIndexDeleted: Number(indexResult.rowCount || 0),
    }
  })
}

export async function getLocalBiometricResetPreview() {
  const result = await queryPostgres(`
    SELECT
      count(*) FILTER (WHERE jsonb_array_length(descriptors) > 0)::integer AS persons_with_descriptors,
      COALESCE(sum(jsonb_array_length(descriptors)), 0)::integer AS total_samples,
      (SELECT count(*)::integer FROM biometric_index) AS biometric_index_entries
    FROM persons
  `)
  const row = result.rows[0] || {}
  return {
    personsWithDescriptors: Number(row.persons_with_descriptors || 0),
    totalSamples: Number(row.total_samples || 0),
    biometricIndexEntries: Number(row.biometric_index_entries || 0),
  }
}
