...
export async function insertLocalAttendanceEntry(attendanceId, entry = {}) {
  await queryPostgres(
    ...
    [
      ...
      entry.latitude != null && Number.isFinite(Number(entry.latitude)) ? Number(entry.latitude) : null,
      entry.longitude != null && Number.isFinite(Number(entry.longitude)) ? Number(entry.longitude) : null,
...
export async function listLocalDtrEmployees(session = {}) {
  const officeId = session?.role === 'hr' || session?.scope === 'office'
    ? normalizeText(session.officeId)
    : ''
