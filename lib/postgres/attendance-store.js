...
function mapOfficeRow(row) {
  ...
  return normalizeOfficeRecord({
    ...
    gps: {
      latitude: row.latitude ?? data?.gps?.latitude,
      longitude: row.longitude ?? data?.gps?.longitude,
      radiusMeters: row.radius_meters ?? data?.gps?.radiusMeters,
    },
  })
}
...
function mapPersonRow(row) {
  ...
  return {
    ...
    active: row.active,
    lifecycleStatus: row.lifecycle_status || (row.approval_status === 'pending' ? 'pending' : (row.active === false ? 'inactive' : 'active')),
...
export async function getLocalAttendanceLogsForDate(employeeId, dateKey, personId = '', { client } = {}) {
  const result = await databaseFor(client).query(
    `
      SELECT *
      FROM attendance
...
export async function getLocalAttendanceLock(identityKey, { client } = {}) {
  const normalizedIdentity = String(identityKey || '').trim()
  if (!normalizedIdentity) return null
  const result = await databaseFor(client).query(
    'SELECT * FROM attendance_locks WHERE employee_id = $1',
    [normalizedIdentity],
  )
  return result.rows[0] || null
}

export async function insertLocalAttendanceEntry(entry, { client } = {}) {
  const identityKey = String(entry.personId || '').trim()
  if (!identityKey) throw new Error('A canonical person ID is required to save attendance.')
  ...
  await databaseFor(client).query(
    `
  ...
  return { attendanceId, storedEntry, entryPreview }
}

export async function upsertLocalAttendanceLock(entry, attendanceId, entryPreview, { client } = {}) {
  const identityKey = String(entry.personId || '').trim()
  if (!identityKey) throw new Error('A canonical person ID is required to update attendance cooldown.')
  await databaseFor(client).query(
    `
