...
export async function getLocalPersonByEmployeeId(employeeId) {
  ...
  const result = await getPostgresPool().query(
    'SELECT * FROM persons WHERE employee_id_lower = $1 LIMIT 1',
    [normalized],
  )
...
export async function getLocalAttendanceLogsForDate(employeeId, dateKey) {
  const result = await getPostgresPool().query(
    `
      SELECT *
      FROM attendance
    ...
    `,
    [String(employeeId || ''), String(dateKey || '')],
  )
...
export async function upsertLocalDailyAttendanceRecord(record) {
  const id = record.id || `${record.employeeId}_${record.dateKey}`
  await getPostgresPool().query(
    `
      INSERT INTO attendance_daily (
        id, employee_id, date_key, name, office_id, office_name,
