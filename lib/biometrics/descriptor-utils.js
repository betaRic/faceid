...
const MIN_REENROLLMENT_DESCRIPTOR_COUNT = 8
const REQUIRED_REENROLLMENT_PHASE = 'chin_down'
const REQUIRED_REENROLLMENT_PHASE_SAMPLE_COUNTS = {
  center: 2,
  side_a: 2,
  side_b: 2,
  chin_down: 2,
...
export function getBiometricReenrollmentAssessment(person, minDiversity = 0.08) {
  ...
  const phaseSampleCounts = metadata.phaseSampleCounts && typeof metadata.phaseSampleCounts === 'object'
    ? metadata.phaseSampleCounts
    : null
  if (!phaseSampleCounts || Object.entries(REQUIRED_REENROLLMENT_PHASE_SAMPLE_COUNTS).some(([phaseId, count]) => {
    return Number(phaseSampleCounts?.[phaseId] || 0) < count
  })) {
    return buildReenrollmentDecision(
      true,
      'missing_pose_support_pairs',
      'Stored face data is missing the current 2-sample support standard for each guided pose.',
    )
  }
