import {
  ...
  CAPTURE_FACE_AREA_TARGET_RATIO,
...
// Face size thresholds use the shared distance-ready band so the oval gate
// cannot accept a face while the UI is still telling the employee to move.
export const OVAL_MIN_FACE_AREA_RATIO = CAPTURE_FACE_AREA_READY_MIN
export const OVAL_MAX_FACE_AREA_RATIO = CAPTURE_FACE_AREA_READY_MAX
...
function getFaceAreaTargetDistance(faceAreaRatio) {
  return Math.abs(Number(faceAreaRatio || 0) - CAPTURE_FACE_AREA_TARGET_RATIO)
}
...
export function selectOvalReadyFace(detections, sourceWidth, sourceHeight) {
  ...
  return detections
    .map(detection => {
      ...
      const targetDistance = getFaceAreaTargetDistance(faceAreaRatio)
      const centerOffset = Math.hypot(centerX - frameCenterX, centerY - frameCenterY) / Math.max(1, Math.hypot(frameCenterX, frameCenterY))
      const centeredness = 1 - Math.min(1, centerOffset)
      const score = (1 - Math.min(1, targetDistance)) + centeredness

      return { detection, box, score, faceAreaRatio, targetDistance, centeredness }
    })
    .filter(Boolean)
    .sort((left, right) => (
      left.targetDistance - right.targetDistance
      || right.centeredness - left.centeredness
