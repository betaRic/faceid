...
import { measureFaceImageQuality } from './image-quality.js'
import { loadWithPipelineIdentity, snapshotPipelineFingerprint } from './pipeline-fingerprint.js'
...
const SERVER_IMAGE_MAX_DIMENSION = 512
const ATTENDANCE_IMAGE_MAX_DIMENSION = 512
const MODEL_BASE_PATH = pathToFileURL(path.join(process.cwd(), 'public', 'models', 'human') + path.sep).href
const WASM_BASE_PATH = path.join(process.cwd(), 'node_modules', '@tensorflow', 'tfjs-backend-wasm', 'wasm-out') + path.sep
const SERVER_ATTENDANCE_ANTISPOOF_ENABLED = process.env.SERVER_ATTENDANCE_ANTISPOOF_ENABLED !== 'false'

let humanInstance = null
let humanLoadPromise = null
const humanPipelineFingerprints = new WeakMap()
...
const HUMAN_PROFILE_CONFIG = {
  attendance: {
    ...
    // server-submitted still frames. Anti-spoofing is authoritative on the
    // server. Temporal mesh is not an attendance gate.
    mesh: false,
  ...
  enrollment: {
    mesh: true,
    antispoof: false,
    maxDimension: SERVER_IMAGE_MAX_DIMENSION,
  },
}
...
async function getServerHuman(profile = 'attendance') {
  ...
  const loadPromise = (async () => {
    ...
    const human = new Human({
      ...
      face: {
        enabled: true,
        detector: { modelPath: 'blazeface.json', rotation: true, minConfidence: 0.5, maxDetected: 5 },
    ...
    // The TF/WASM backend is shared across Human instances. If another profile
    // initialized it first, disk hashes cannot prove that already-loaded runtime.
    const ownsRuntimeInitialization = !runtimeInitializationStarted && human.tf.getBackend() !== 'wasm'
    runtimeInitializationStarted = true
    let modelLoadFinished = false
    const { pipelineFingerprint } = await loadWithPipelineIdentity({
      enabled: ownsRuntimeInitialization && normalizedProfile === 'attendance' && process.env.BIOMETRIC_MEMORY_COLLECT_ENABLED === 'true',
      snapshot: () => {
        if (modelLoadFinished) {
          const loaded = human.models.loaded()
          const required = ['blazeface', 'faceres', ...(profileConfig.antispoof ? ['antispoof'] : [])]
          if (!required.every(name => loaded.includes(name))) throw new Error('Pipeline models not fully loaded')
        }
        return snapshotPipelineFingerprint({
        config: human.config,
        modelBasePath: fileURLToPath(MODEL_BASE_PATH),
        wasmBasePath: WASM_BASE_PATH,
        versions: {
          human: human.version,
          tfjsCore: human.tf.version?.['tfjs-core'],
          tfjsBackendWasm: human.tf.version?.['tfjs-backend-wasm'],
          sharp: sharp.versions,
        },
        preprocessing: {
          version: 'server-rgb-face-quality-v1',
          inputPixelLimit: 4_000_000,
          autoRotate: true,
          maxDimension: profileConfig.maxDimension,
          resizeFit: 'inside',
          withoutEnlargement: true,
          removeAlpha: true,
          output: 'raw-rgb-int32',
          descriptor: 'l2-normalized-v1',
          quality: 'face-roi-brightness-clipping-laplacian-v1',
        },
        })
      },
      load: async () => { await human.load(); modelLoadFinished = true; return human },
    })
    humanPipelineFingerprints.set(human, pipelineFingerprint)
...
async function decodeFrameToTensor(human, frameDataUrl, options = {}) {
...
export async function generateServerFaceEmbedding(frameDataUrl, options = {}) {
  const profile = normalizeHumanProfile(options.profile)
  const human = await getServerHuman(profile)
  const tensor = await decodeFrameToTensor(human, frameDataUrl, {
    maxDimension: HUMAN_PROFILE_CONFIG[profile].maxDimension,
  })
  ...
  try {
    ...
    // Optional provenance or image-quality work must never fail attendance.
    const pipelineFingerprint = profile === 'attendance' && process.env.BIOMETRIC_MEMORY_COLLECT_ENABLED === 'true'
      ? humanPipelineFingerprints.get(human) ?? null : null
    let quality = null
    if (pipelineFingerprint) {
      try {
        quality = measureFaceImageQuality(await tensor.data(), tensor.shape[1], tensor.shape[0], {
          x: face.box?.[0], y: face.box?.[1], width: face.box?.[2], height: face.box?.[3],
        })
      } catch { /* Missing optional evidence prevents collection, not attendance. */ }
    }
    return {
      ok: true,
      pipelineFingerprint,
      ...
      face: {
        ...
        quality,
        antispoof: HUMAN_PROFILE_CONFIG[profile].antispoof ? (face.real ?? null) : null,
...
export function generateServerEnrollmentSupportEmbedding(frameDataUrl) {
...
export async function warmServerAttendanceEmbedding() {
  await getServerHuman('attendance')
  return true
}
