...
const tfjsBackendWasm = require('@tensorflow/tfjs-backend-wasm')
...
const SERVER_ATTENDANCE_PAD_ENABLED = process.env.SERVER_ATTENDANCE_PAD_ENABLED === 'true'
...
let wasmBackendConfigured = false
...
const HUMAN_PROFILE_CONFIG = {
  attendance: {
    ...
    // in the browser burst, so loading server mesh/iris/PAD here only adds
    // latency unless SERVER_ATTENDANCE_PAD_ENABLED is explicitly enabled.
    mesh: false,
    iris: false,
    antispoof: SERVER_ATTENDANCE_PAD_ENABLED,
    liveness: SERVER_ATTENDANCE_PAD_ENABLED,
...
function configureServerWasmBackend() {
  if (wasmBackendConfigured) return
  tfjsBackendWasm.setWasmPaths(WASM_BASE_PATH)
  wasmBackendConfigured = true
}
