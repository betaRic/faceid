...
import { DESCRIPTOR_LENGTH as EXPECTED_DESCRIPTOR_LENGTH } from '@/lib/config'
...
export function summarizeEnrollmentCaptureQuality(metrics = {}) {
  ...
  if (faceAreaRatio < 0.10) {
    warnings.push('Move much closer — face is too small.')
  } else if (faceAreaRatio < 0.40) {
    warnings.push('Move closer — face should fill the oval.')
  } else if (faceAreaRatio > 0.92) {
    warnings.push('Move back — face is too close to camera.')
  }
