 ...
 *
 * Design (revised):
 * - Human's RGB antispoof model is noisy in real webcam/mobile conditions.
 *   Treat only very low scores as hard spoof evidence; gray-zone scores are
 *   risk flags, not employee-blocking proof.
 * - Strong liveness means ONE eye-based signal AND ONE motion-based signal:
 *     * Eye-based:    blink OR EAR variance
 *     * Motion-based: mesh micro-movement OR relative iris motion
 ...
 *   Missing motion alone is flagged because normal users can hold still, but
 *   missing eye evidence while iris position is static is blocked as photo-like.
 * - Iris motion tracks MediaPipe iris landmarks (mesh indices 468-477). Real
 *   eyes micro-saccade relative to the face; a printed photo's iris only moves
 *   with the rigid paper/screen, so absolute screen-space movement is not enough.
 *
...
// We track residual movement relative to the face anchors.
const IRIS_LANDMARKS = [468, 473]
const FACE_ANCHORS = [33, 263]
...
// PAD thresholds
const ANTISPOOF_HARD_MIN = 0.3  // clear spoof floor; gray-zone scores are risk flags
const ANTISPOOF_GRAY_MIN = 0.58
...
function getFaceAnchorFrame(mesh) {
  const left = getMeshPoint(mesh, FACE_ANCHORS[0])
  const right = getMeshPoint(mesh, FACE_ANCHORS[1])
  if (!left || !right) return null

  const scale = dist2d(left, right)
...
function getFaceRelativePoint(point, frame) {
  if (!point || !frame) return null
  return {
    x: (point.x - frame.center.x) / frame.scale,
    y: (point.y - frame.center.y) / frame.scale,
  }
}

export function computeIrisDelta(meshA, meshB) {
  if (!meshA || !meshB) return null
  const frameA = getFaceAnchorFrame(meshA)
  const frameB = getFaceAnchorFrame(meshB)
  if (!frameA || !frameB) return null

  const averageScale = (frameA.scale + frameB.scale) / 2
  let total = 0
  let count = 0
  for (const idx of IRIS_LANDMARKS) {
    const a = getMeshPoint(meshA, idx)
    const b = getMeshPoint(meshB, idx)
    const relA = getFaceRelativePoint(a, frameA)
    const relB = getFaceRelativePoint(b, frameB)
    const d = dist2d(relA, relB)
    if (d !== null) { total += d; count++ }
  }
  return count > 0 ? (total / count) * averageScale : null
}
...
function getLivenessRiskFlags({ avgAntispoof, avgLiveness, signals }) {
  const flags = []
  if (avgAntispoof !== null && avgAntispoof < ANTISPOOF_GRAY_MIN) flags.push('pad_gray_zone')
  if (avgLiveness !== null && avgLiveness < LIVENESS_SOFT_MIN) flags.push('weak_human_liveness_score')
  if (!signals.eye) flags.push('weak_eye_signal')
  if (!signals.motion) flags.push('weak_motion_signal')
  return flags
}
...
export function analyzeBurstLiveness(frames) {
  ...
  // signal against printed photos and phone screens, but it is not calibrated
  // enough for a high threshold. Fail only on a very low score.
  ...
  if (!signals.eye && (avgIrisDelta === null || avgIrisDelta < IRIS_MOTION_LIVE_MIN)) {
    return buildResult({
      earSamples, meshDeltas, irisDeltas, blinkCount, earVar, avgDelta,
      avgIrisDelta, avgAntispoof, avgLiveness, signals,
      frameCount: frames.length, score: 0, pass: false, reason: 'photo_like_rigid_motion',
    })
  }

  // Scoring for diagnostics. Missing eye/motion evidence becomes a risk flag
  // instead of a hard employee-blocking failure.
  let score = signals.eye && signals.motion ? 0.40 : 0.20
  ...
  const riskFlags = getLivenessRiskFlags({ avgAntispoof, avgLiveness, signals })
  ...
  return buildResult({
    ...
    score,
    pass: true,
    reason: riskFlags.length > 0 ? 'weak_liveness_signal' : '',
    riskFlags,
  })
}
...
export function validateLivenessEvidence(evidence) {
  ...
  const signals = summarizeSignals({ blinkCount, earVar, avgDelta, avgIrisDelta })
  if (!signals.eye && (avgIrisDelta === null || avgIrisDelta < IRIS_MOTION_LIVE_MIN)) {
    return { ok: false, reason: 'photo_like_rigid_motion' }
  }
  ...
  return {
    ok: true,
    hasEyeSignal: signals.eye,
    hasMotionSignal: signals.motion,
    blinkCount,
    earVariance: earVar,
