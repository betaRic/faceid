 ...
 *
 * Uses landmark micro-movement variance across frames.
 * A live face has natural micro-movement (head sway, breathing).
 * A printed photo or frozen frame has near-zero variance.
 *
 * Compatible with @vladmandic/human mesh format: each point is [x, y, z] or {x, y}.
 *
 * Policy:
 *   - If we have enough frames and detect STATIC face → block (definite spoof signal)
 *   - If we have too few frames → allow (cannot determine, assume live)
 *   - If movement is excessive → allow (unstable capture, not a spoof)
 */
