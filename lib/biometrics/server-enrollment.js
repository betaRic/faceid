...
function selectRequiredPoseFrames(frames) {
  const byPhase = new Map()
  for (const frame of frames) {
    const phaseId = String(frame?.phaseId || '').trim()
    if (!ENROLLMENT_REQUIRED_PHASE_IDS.includes(phaseId) || byPhase.has(phaseId)) continue
    byPhase.set(phaseId, frame)
  }
  return ENROLLMENT_REQUIRED_PHASE_IDS
    .map(phaseId => byPhase.get(phaseId))
    .filter(Boolean)
}
...
export async function buildAuthoritativeEnrollmentPayload(sampleFrames, captureMetadata) {
  ...
  const requiredPoseFrames = selectRequiredPoseFrames(normalizedFrames)
