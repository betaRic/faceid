...
async function getExistingOpenVinoProfileSampleCount(db, personId) {
  const { getLocalPersonById } = await import('@/lib/postgres/person-store')
  const person = await getLocalPersonById(personId)
  return finiteNumber(person?.openvinoProfileSampleCount, person?.data?.openvinoProfileSampleCount) || 0
}
...
export async function writeOpenVinoProfileSamples(db, {
  ...
  const { cosineDistance, OPENVINO_RETAIL_MODEL_VERSION } = await import('@/lib/biometrics/openvino-retail-embedding')
  ...
  const { getLocalPersonById } = await import('@/lib/postgres/person-store')
  const { withPostgresTransaction } = await import('@/lib/postgres/client')
  return withPostgresTransaction(async client => {
    const existingPerson = await getLocalPersonById(personId)
    const data = existingPerson?.data || {}
    ...
    await client.query(
      ...
      [
        ...
        JSON.stringify({
          ...
          openvinoProfileUpdatedAt: profile.updatedAt,
        }),
      ],
    )
    ...
    return {
      ...
      openvinoProfile: profile,
      openvinoEmbeddings: trimmed,
      openvinoUpdatedAt: profile.updatedAt,
    }
  })
}
