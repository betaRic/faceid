...
import { aggregateServerAntispoof } from '@/lib/biometrics/antispoof-policy'
...
function buildAttendanceError(message, decisionCode = 'blocked_no_reliable_match', status = 403) {
  return new SafeRequestError(message, { code: decisionCode, status })
}
...
function buildPayloadFromAcceptedFrames(acceptedFrames, rejectedFrames, processedCount) {
  ...
  const antispoof = aggregateServerAntispoof(acceptedFrames)
...
export async function buildAuthoritativeAttendancePayload(scanFrames, options = {}) {
  ...
  for (const frame of normalizedFrames.slice(processedCount)) {
    ...
    acceptedFrames.push({
      ...
      pipelineFingerprint: embedding.pipelineFingerprint ?? null,
      antispoof: embedding.face?.antispoof ?? null,
      quality: embedding.face?.quality ? {
        ...embedding.face.quality,
        faceScore: embedding.face.score,
        faceWidth: embedding.face.box?.width,
        faceHeight: embedding.face.box?.height,
      } : null,
