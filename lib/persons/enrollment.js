...
export function deduplicateDescriptors(incomingDescriptors, existingDescriptors = [], options = {}) {
  ...
  for (const raw of incomingDescriptors) {
    ...
    const tooCloseToBatch = accepted.some(
      acc => euclideanDistance(normalizeDescriptor(acc), normalized) < minSampleDiversity,
    )
    if (tooCloseToBatch) {
...
/**
 * Same as checkDuplicateFace but reads within a database transaction.
...
export async function enrollPerson(db, body, office, resolvedSession) {
  ...
  const transactionResult = await db.runTransaction(async transaction => {
    ...
    if (existingStoredDescriptors.length + uniqueDescriptors.length < ENROLLMENT_MIN_SAMPLES) {
      throw Object.assign(
        new Error(
          `Enrollment must save at least ${ENROLLMENT_MIN_SAMPLES} validated support samples before the profile can be used for attendance.`,
        ),
