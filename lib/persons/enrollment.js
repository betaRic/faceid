...
export async function enrollPerson(db, body, office, resolvedSession) {
  const payload = {
    ...
    biometricQualityScore: Number.isFinite(body.captureMetadata?.qualityScore) ? Number(body.captureMetadata.qualityScore) : null,
  ...
  try {
    ...
    if (!indexSyncWarning) {
      indexSyncWarning = 'Enrollment saved but biometric mirror sync failed. Admin should review the biometric mirror.'
    }
  }
