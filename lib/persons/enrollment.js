...
/**
 * Fetch ALL active+approved/pending biometric index entries as candidate objects.
 * Returns lightweight objects with only what's needed for L2 comparison:
 * { personId, normalizedDescriptor }
 *
 * Bypasses bucket filtering intentionally — see module header.
 */
async function fetchAllDuplicateCandidates(db) {
  ...
  for (const snapshot of snapshots) {
    for (const doc of snapshot.docs) {
      ...
      const nd = Array.isArray(data.normalizedDescriptor)
        ? data.normalizedDescriptor.map(Number)
        : []
      // Skip entries without a valid descriptor in the index
      if (nd.length === 0) continue
      candidates.push({ personId: data.personId, normalizedDescriptor: nd })
    }
  }
...
/**
 * Same as fetchAllDuplicateCandidates but reads within a Firestore transaction.
 * Maintains snapshot isolation so concurrent enrollments can't race past the check.
 */
async function fetchAllDuplicateCandidatesWithinTransaction(db, transaction) {
  const snapshots = await Promise.all(
    DUPLICATE_BLOCK_APPROVALS.map(approvalStatus =>
      transaction.get(
        db.collection(BIOMETRIC_INDEX_COLLECTION)
 ...
 *
 * Performs a full index scan — no bucket filtering — so that different capture
 * conditions (lighting, pose, distance) cannot cause a same-face to slip through.
 *
...
export async function checkDuplicateFace(db, descriptors, excludePersonId = '') {
  ...
  const candidates = await fetchAllDuplicateCandidates(db)
  if (candidates.length === 0) return null

  for (const descriptor of descriptors) {
    const normalizedQuery = normalizeDescriptor(descriptor)

    for (const candidate of candidates) {
      if (candidate.personId === excludePersonId) continue

      const distance = euclideanDistance(candidate.normalizedDescriptor, normalizedQuery)
      if (distance < DISTANCE_THRESHOLD_ENROLLMENT) {
        // Fetch the persons doc only when we have a confirmed match
...
export async function checkDuplicateFaceWithinTransaction(
  ...
  const candidates = await fetchAllDuplicateCandidatesWithinTransaction(db, transaction)
  if (candidates.length === 0) return null
...
export async function enrollPerson(db, body, office, resolvedSession) {
  const payload = {
    ...
    biometricModelVersion: String(body.captureMetadata?.modelVersion || 'human-faceres-browser-v1'),
  ...
  // Pre-transaction duplicate face check (full scan, outside transaction).
  // Catches the common case before we open a transaction.
  // The transaction below repeats the check for concurrent-enrollment safety.
  // Exclude the current employee record when resubmitting samples for the same person.
  const earlyDuplicate = await checkDuplicateFace(db, body.descriptors, earlyDuplicateExcludePersonId)
  if (earlyDuplicate) {
