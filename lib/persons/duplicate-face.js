...
export const EXACT_PENDING_DUPLICATE_DISTANCE_MAX = 1e-12
...
export function findExactPendingDuplicateCandidate(candidates, descriptors, excludePersonId = '') {
  for (const person of candidates || []) {
    if (!person?.id || person.id === excludePersonId) continue
    if (getEffectivePersonApprovalStatus(person) !== PERSON_APPROVAL_PENDING) continue

    const snapshot = buildDuplicateFaceSnapshot(person, descriptors)
    if (
      snapshot?.reviewRequired
      && Number.isFinite(snapshot.bestDistance)
      && snapshot.bestDistance <= EXACT_PENDING_DUPLICATE_DISTANCE_MAX
    ) {
