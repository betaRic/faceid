...
export async function getEmployeeDailyAttendanceRecord(db, employeeId, dateKey) {
  ...
  if (postgresEnabled()) {
    const result = await queryPostgres(
      'SELECT id, data FROM attendance_daily WHERE employee_id = $1 AND date_key = $2 LIMIT 1',
      [normalizedEmployeeId, normalizedDateKey],
    )
...
export async function listEmployeeDailyAttendanceRecords(db, employeeId) {
  if (postgresEnabled()) {
    const result = await queryPostgres(
      'SELECT id, data FROM attendance_daily WHERE employee_id = $1 ORDER BY date_key ASC',
      [String(employeeId || '')],
    )
    return result.rows.map(row => normalizeDailyRecord({ id: row.id, ...row.data })).filter(Boolean)
  }
