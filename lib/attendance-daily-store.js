...
function normalizeDailyRecord(record, fallbackDateKey = '') {
  ...
  return {
    ...
    date: dateKey,
    dateLabel,
    amInTimestamp: Number(data.amInTimestamp ?? 0) || null,
    amOutTimestamp: Number(data.amOutTimestamp ?? 0) || null,
    pmInTimestamp: Number(data.pmInTimestamp ?? 0) || null,
    pmOutTimestamp: Number(data.pmOutTimestamp ?? 0) || null,
    amIn: String(data.amIn || '--'),
    amOut: String(data.amOut || '--'),
    pmIn: String(data.pmIn || '--'),
    pmOut: String(data.pmOut || '--'),
