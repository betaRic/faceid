...
export function getRegion12Blueprint() {
  ...
  return {
    ...
    sampleEmployees,
    nextQuestions: [
      'Should registration also require an employee ID or personnel number to avoid duplicate names?',
      'Do you want the transfer history visible in the admin dashboard?',
      'Should weekends and holidays be configurable separately per office?',
      'Should employees see their last GPS validation result in the mobile app?',
    ],
  }
}
