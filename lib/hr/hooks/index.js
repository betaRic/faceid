...
function createHrStore() {
  return create((set, get) => ({
    ...
    setEmployeeQuery: (employeeQuery) =>
      set({ employeeQuery, employeePage: 1 }),
    setEmployeeOfficeFilter: (employeeOfficeFilter) =>
      set({ employeeOfficeFilter, employeePage: 1 }),
    setEmployeeStatusFilter: (employeeStatusFilter) =>
      set({ employeeStatusFilter, employeePage: 1 }),
    setEmployeePage: (employeePage) => set({ employeePage }),
    setEmployeePagination: (total, hasMore) =>
      set({ employeeTotal: total, employeeHasMore: hasMore }),
  }));
}
...
export function useHrSession() {
  ...
  const loginWithEmailPin = useCallback(
    async (email, pin) => {
      setLoading(true);
      try {
        const res = await fetch("/api/hr/login", {
