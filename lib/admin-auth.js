...
async function isRegionalPinBootstrapAvailable(db) {
  return isRegionalPinConfigured() && await isRegionalPinEnabled(db)
}
...
export async function resolveAdminSession(db, session) {
  ...
  if (isRegionalPinSession(session)) {
    return await isRegionalPinBootstrapAvailable(db)
      ? {
          ...session,
          scope: 'regional',
          officeId: '',
