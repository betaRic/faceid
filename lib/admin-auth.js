...
import { isRegionalPinEnabled } from './bootstrap-pin'
...
export function createAdminSessionCookieValue(session = {}) {
  ...
  const payload = {
    ...
    authMethod: session.authMethod === 'shared_regional_pin' ? 'shared_regional_pin' : 'named_pin',
    iat: now,
...
export function parseAdminSessionCookieValue(cookieValue) {
  ...
  try {
    ...
    return {
      ...
      authMethod: payload.authMethod === 'shared_regional_pin'
        || (!payload.authMethod && isRegionalPinSession(payload))
        ? 'shared_regional_pin'
        : 'named_pin',
      iat: payload.iat,
      exp: payload.exp,
...
async function isRegionalPinBootstrapAvailable() {
  return isRegionalPinConfigured() && await isRegionalPinEnabled()
}
...
export async function resolveAdminSession(db, session) {
  ...
  if (isRegionalPinSession(session)) {
    return await isRegionalPinBootstrapAvailable()
      ? {
          ...session,
          scope: 'regional',
          officeId: '',
