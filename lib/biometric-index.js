...
const MULTI_DESCRIPTOR_UNCERTAIN_DISTANCE = 0.78
  ...
  return Array.from(aggregates.values())
    .map(person => {
      ...
      return {
        ...
        requiresSupport: Number.isFinite(bestDistance) && Number(bestDistance) >= MULTI_DESCRIPTOR_UNCERTAIN_DISTANCE,
      }
    })
...
export function matchBiometricIndexMultiDescriptor(candidateSamples, descriptors, distanceThreshold, ambiguousMargin) {
  ...
  const viableRanked = ranked.filter(candidate => (
    !candidate.requiresSupport || candidate.supportCount >= MULTI_DESCRIPTOR_REQUIRED_SUPPORT
  ))
  ...
  const debug = {
    ...
    supportGate: best
      ? ''
      : bestRaw?.requiresSupport && bestRaw?.supportCount < MULTI_DESCRIPTOR_REQUIRED_SUPPORT
