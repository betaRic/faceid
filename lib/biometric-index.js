...
async function tryGetCachedCandidates(officeIds, bucketA, bucketB) {
  try {
    ...
    for (let i = 0; i < cached.length; i++) {
      ...
      try {
        const officeData = JSON.parse(cached[i])
        const officeCandidates = []
        if (officeData[bucketA]) officeCandidates.push(...officeData[bucketA])
        if (officeData[bucketB]) officeCandidates.push(...officeData[bucketB])
        for (const c of officeCandidates) {
          if (c.approvalStatus === 'approved' && c.active !== false) {
            candidates.push(c)
          }
        }
      } catch {}
...
async function cacheCandidatesForOffice(officeId, allSamples) {
  try {
    ...
    await kv.set(`${KV_CACHE_PREFIX}${officeId}`, JSON.stringify(byBucket), { ex: KV_CACHE_TTL_SECONDS })
  } catch (err) {
    console.error('Biometric index cache write failed:', err)
  }
}

async function warmCacheForOffices(db, officeIds) {
  try {
    ...
    console.error('Biometric cache warm failed:', err)
  }
}
...
export async function queryBiometricIndexCandidates(db, officeIds, descriptor) {
  ...
  if (uniqueOfficeIds.length <= 5) {
    await warmCacheForOffices(db, uniqueOfficeIds)
  }
...
candidateSamples.forEach(sample => {
    const sampleDescriptor = Array.isArray(sample.normalizedDescriptor) 
      ...
      : [];
    if (sampleDescriptor.length !== queryDescriptor.length) return;
