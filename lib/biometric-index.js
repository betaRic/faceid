...
function isIndexSampleSearchable(sample) {
  ...
  // Backward-compatibility: older index rows may have stale biometricEnabled=false
  // even when the underlying person is active and approved. Allow those rows so
  // kiosk matching still works until the index is rebuilt.
...
export async function syncPersonBiometricIndex(db, personId, personData) {
  ...
  if (officeId) {
    invalidateOfficeCandidateCache([officeId])
  }
}
...
export async function deletePersonBiometricIndex(db, personId) {
  ...
  const officeIds = snapshot.docs.map(record => String(record.data()?.officeId || ''))
  const batch = db.batch()
  snapshot.docs.forEach(record => batch.delete(record.ref))
  await batch.commit()
  invalidateOfficeCandidateCache(officeIds)
}
...
export async function queryBiometricIndexCandidates(db, officeIds, descriptor) {
  ...
  // Try KV cache first — returns ALL candidates per office, no bucket filtering
  const cached = await tryGetCachedCandidates(uniqueOfficeIds)
  if (cached && cached.length > 0) {
    return filterCandidatesByBuckets(cached, bucketA, bucketB)
  }

  // Firestore fallback — fetch ALL active+approved entries for these offices
  const snapshots = []
  for (let i = 0; i < uniqueOfficeIds.length; i += FIRESTORE_IN_QUERY_CHUNK_SIZE) {
    const chunk = uniqueOfficeIds.slice(i, i + FIRESTORE_IN_QUERY_CHUNK_SIZE)
    snapshots.push(
  ...
  return filterCandidatesByBuckets(Array.from(deduped.values()), bucketA, bucketB)
}
