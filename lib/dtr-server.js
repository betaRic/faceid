...
async function loadAttendanceLogsForMonth(db, personId, startDate, endDate) {
  const result = await queryPostgres(
      ...
      `,
      [personId, startDate.getTime(), endDate.getTime()],
    )
...
export async function buildEmployeeDtrDocument(db, {
  ...
  const logs = await loadAttendanceLogsForMonth(db, person.id || '', startDate, endDate)
