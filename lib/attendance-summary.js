...
export function buildAttendanceSummary({ attendance, persons, offices, targetDate }) {
  ...
  const personsByEmployeeId = new Map()
  persons.forEach(person => {
    const key = String(person.employeeId || '').trim()
    if (!key) return
    const matches = personsByEmployeeId.get(key) || []
    matches.push(person)
    personsByEmployeeId.set(key, matches)
  })
  ...
  return Array.from(grouped.entries())
    .map(([, logs]) => {
      ...
      const person = personsById.get(String(firstLog.personId || '').trim())
        || ((personsByEmployeeId.get(String(firstLog.employeeId || '').trim()) || []).length === 1
          ? personsByEmployeeId.get(String(firstLog.employeeId || '').trim())[0]
          : null)
