...
function writeDtrCopy(sheetXml, dtr, copy) {
  ...
  output = setCell(output, copy.employeeName, employee.name || '', copy.employeeName.startsWith('M') ? '51' : '48')
  ...
  for (const row of dtr?.rows || []) {
    ...
    output = setCell(output, `${copy.amIn}${rowNumber}`, row.isActive ? row.amIn || '' : '', styles.amIn)
    output = setCell(output, `${copy.amOut}${rowNumber}`, row.isActive ? row.amOut || '' : '', styles.amOut)
    output = setCell(output, `${copy.pmIn}${rowNumber}`, row.isActive ? row.pmIn || '' : '', styles.pmIn)
    output = setCell(output, `${copy.pmOut}${rowNumber}`, row.isActive ? row.pmOut || '' : '', styles.pmOut)
  }

  return output
}

function buildSheetXml(templateSheetXml, dtr) {
  return COPY_DEFINITIONS.reduce((xml, copy) => writeDtrCopy(xml, dtr, copy), templateSheetXml)
}
...
function replaceSheetsXml(workbookXml, sheets) {
  ...
  const definedNamesXml = `<definedNames>${sheets
    .map((sheet, index) => (
      `<definedName name="_xlnm.Print_Area" localSheetId="${index}">${sheetNameForFormula(sheet.name)}!$B$1:$W$58</definedName>`
    ))
    .join('')}</definedNames>`
...
export function buildDtrWorkbookFromTemplate(templateBytes, dtrs = []) {
  ...
  for (const sheet of sheets) {
    files[sheet.path] = strToU8(buildSheetXml(templateSheetXml, sheet.dtr))
