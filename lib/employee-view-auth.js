...
export function createEmployeeViewSessionCookieValue(session = {}) {
  ...
  if (!personId) throw new Error('Person ID is required for employee view sessions')
...
export function employeeViewSessionMatchesPerson(session, personId) {
  const normalizedPersonId = String(personId || '').trim()
  return Boolean(normalizedPersonId && String(session?.personId || '').trim() === normalizedPersonId)
}
...
export async function resolveEmployeeViewSessionRequest(request, db) {
  ...
  return parseEmployeeViewSessionCookieValue(requestValue)
}
