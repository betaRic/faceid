...
function summarizeGroup(events) {
  ...
  const databaseReadMs = collectMetrics(events, event => event?.performance?.databaseReadMs)
  const databaseWriteMs = collectMetrics(events, event => event?.performance?.databaseWriteMs)
  ...
  return {
    ...
    medianDatabaseReadMs: percentile(databaseReadMs, 0.5),
    p95DatabaseReadMs: percentile(databaseReadMs, 0.95),
    medianDatabaseWriteMs: percentile(databaseWriteMs, 0.5),
    p95DatabaseWriteMs: percentile(databaseWriteMs, 0.95),
