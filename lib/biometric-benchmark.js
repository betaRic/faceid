...
function summarizeGroup(events) {
  ...
  const activeChallengeEvents = events.filter(event => String(event?.challenge?.mode || '').toLowerCase() === 'active')
  ...
  return {
    ...
    activeChallengeRate: rate(activeChallengeEvents.length, total),
  ...
  return Array.from(groups.entries())
    .map(([key, groupEvents]) => {
      ...
      return {
        ...
        activeChallengeRate: summary.activeChallengeRate,
      }
    })
...
export function buildBiometricBenchmarkReport(scanEvents = [], options = {}) {
  ...
  const activeChallengeEvents = scanEvents.filter(event => String(event?.challenge?.mode || '').toLowerCase() === 'active')
  const activeWfhEvents = activeChallengeEvents.filter(event => String(event?.attendanceMode || '').toUpperCase() === 'WFH')
  const acceptedActiveWfhEvents = activeWfhEvents.filter(event => event.status === 'accepted')
  ...
  const deploymentHealth = {
    ...
    wfhChallengePassRate: rate(acceptedActiveWfhEvents.length, activeWfhEvents.length),
    activeChallengeRate: rate(activeChallengeEvents.length, total),
