...
export function createPollingSubscription(load, onData, onError, intervalMs = DEFAULT_POLL_INTERVAL_MS) {
  ...
  const run = async () => {
    ...
    try {
      ...
      failCount++
      // Auth failures are terminal: the session is gone. No amount of retrying
      // fixes that. Stop the subscription so we don't hammer the server (and
      // spam the console) every N minutes until the tab is closed.
