...
function getOpenVinoReadiness() {
  const modelRoot = process.env.OPENVINO_MODEL_DIR
    ? path.resolve(process.env.OPENVINO_MODEL_DIR)
    : path.join(/*turbopackIgnore: true*/ process.cwd(), 'public', 'models', 'openvino')
...
export function getRuntimeReadiness() {
  ...
  if (openvino.shadowEnabled) {
    if (!openvino.modelsAvailable) {
      warnings.push('OpenVINO shadow collection is enabled but retail model files are missing. Run npm run openvino:download-models before Railway/NAS deployment.')
    }
    if (openvino.framesPerScan < 2) {
      warnings.push('OPENVINO_SHADOW_FRAMES_PER_SCAN is below 2. Use 2 so accepted scans can collect both submitted still frames during shadow migration.')
    }
