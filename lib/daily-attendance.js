...
function computeSegmentTimes(logs, office) {
  ...
  logs
    ...
    .forEach(log => {
      ...
      // Treat scans between lunch boundaries as the morning exit.
 ...
 * 'complete' — full day (AM in+out, PM in+out) already recorded; further scans should be blocked
 *
 * This is used by the attendance route to set entry.action and to block scans after a complete day.
 */
export function getNextAttendanceAction(logs, office) {
  ...
  // Full AM+PM day recorded. Block further scans at the route level.
  return 'complete'
}
