...
import { REQUIRED_DATABASE_UPDATES } from './required-database-updates.js'
...
async function loadMigrationEvidence({ query, requiredUpdates }) {
  try {
    const expected = [...requiredUpdates].sort()
    ...
    return {
      status: pending.length ? 'failing' : 'healthy',
...
export async function buildSystemEvidence({
  ...
  access = fsAccess,
  readFile = fsReadFile,
  requiredUpdates = REQUIRED_DATABASE_UPDATES,
  ...
  const [database, migrations, dailySummary, buildId] = await Promise.all([
    ...
    loadMigrationEvidence({ query, requiredUpdates }),
  ...
  if (migrations.status !== 'healthy') {
    actions.push({
      ...
      title: migrations.status === 'failing' ? 'Database update required' : 'Database update check unavailable',
      detail: migrations.status === 'failing'
        ? `${migrations.pending.length} required database update(s) are not installed.`
        : 'Installed database updates could not be checked.',
    })
  }
