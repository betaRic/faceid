...
const DECISION_CATEGORY = new Map([
  ...
  ['blocked_antispoof', 'antispoof_observation_block'],
  ['blocked_missing_antispoof', 'antispoof_observation_block'],
  ['blocked_liveness', 'other_biometric_failure'],
  ['blocked_missing_liveness', 'other_biometric_failure'],
  ['blocked_photo_detected', 'other_biometric_failure'],
  ['blocked_photo_detected_flat', 'other_biometric_failure'],
  ['blocked_photo_detected_flat_no_blink', 'other_biometric_failure'],
...
function buildFailuresByPerson(events, currentEmployees) {
  const peopleById = new Map(currentEmployees.map(employee => [normalizedText(employee?.personId || employee?.id), employee]))
  const groups = new Map()
  ...
  const failuresByPerson = [...groups.values()]
    .map(group => {
      const person = peopleById.get(group.identity.personId)
      const decisionCounts = new Map()
      group.events.forEach(event => {
        const key = normalizedText(event.decisionCode) || 'unknown'
        decisionCounts.set(key, (decisionCounts.get(key) || 0) + 1)
      })
      return {
        personId: group.identity.personId,
        employeeId: normalizedText(person?.employeeId || group.identity.employeeId),
        name: normalizedText(person?.name || group.events.find(event => normalizedText(event.name))?.name) || 'Unknown employee',
    ...
    .sort((left, right) => right.attempts - left.attempts || left.name.localeCompare(right.name))

  return { failuresByPerson, unattributedFailureCount }
}
...
export function buildMaintenanceEvidenceReport(scanEvents = [], options = {}) {
  ...
  const failureOwnership = buildFailuresByPerson(events, currentEmployees)
  const evidence = {
    totalWindowEvents,
  ...
  return {
    ...
    failuresByPerson: failureOwnership.failuresByPerson,
    unattributedFailureCount: failureOwnership.unattributedFailureCount,
    breakdowns: {
      categories,
