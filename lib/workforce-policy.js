...
export function postgresDateKey(value) {
...
export function resolveDtrSpecialDay({ dateKey, holidays = [], leaves = [], orders = [], person }) {
  const holiday = holidays.find(row => row.holidayDate === dateKey && (
    row.scopeType === 'national' || (row.scopeType === 'office' && row.officeId === person?.officeId) || (row.scopeType === 'division' && row.officeId === person?.officeId && row.divisionId === person?.divisionId)
  ))
...
export async function loadWorkforcePolicies() {
...
export async function loadDtrWorkforceRecords({ person, startDate, endDate }) {
  ...
  const map = row => ({ ...row, scopeType: row.scope_type, scopeId: row.scope_id, weeklySchedule: row.weekly_schedule || {}, holidayDate: postgresDateKey(row.holiday_date), officeId: row.office_id, divisionId: row.division_id, leaveType: row.leave_type, startDate: postgresDateKey(row.start_date), endDate: postgresDateKey(row.end_date) })
