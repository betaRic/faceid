import 'server-only'
import { kvIncrWithExpire } from './kv-utils'

/**
 * Rate limiting via local in-memory counters.
 * Falls back to in-memory Map when KV is not available. This resets on cold
 * start and avoids burning database reads/writes on rate limit checks.
 */
...
export async function enforceRateLimit(db, { key, limit, windowMs }) {
  ...
  const kvKey = `rl:${normalizedKey}:${windowStart}`
  const count = await kvIncrWithExpire(kvKey, windowSeconds + 60)
  if (count === null) {
