...
import { resolvePersistedScanIdentity } from '@/lib/maintenance/event-evidence'
...
export async function writeScanEvent(_db, options) {
  try {
    await writeLocalScanEvent(buildScanEventRecord(options))
    return { ok: true }
  } catch (error) {
    const errorId = randomUUID()
    console.error('[scan-events] Failed to write rejected scan history', {
      errorId,
