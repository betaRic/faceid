...
function summarizeServerPerformance(serverTimings) {
  ...
  return {
    ...
    firestoreReadMs: sumTimings(serverTimings, ['offices', 'office', 'daily_logs']),
    firestoreWriteMs: sumTimings(serverTimings, ['write_attendance', 'daily_cache', 'scan_event', 'employee_session']),
  ...
  try {
    ...
    await db.collection('scan_events').add({
      ...
      captureContext: {
        ...
        authoritativeDescriptorSource: String(captureContext.authoritativeDescriptorSource || '').slice(0, 80),
        serverEmbeddingFrames: Number.isFinite(captureContext.serverEmbeddingFrames) ? Number(captureContext.serverEmbeddingFrames) : null,
        serverEmbeddingAverageMs: Number.isFinite(captureContext.serverEmbeddingAverageMs) ? Number(captureContext.serverEmbeddingAverageMs) : null,
      },
      scanDiagnostics: {
        serverMatchMode: String(scanDiagnostics.serverMatchMode || '').slice(0, 40),
        ...
        serverDescriptorSpread: roundMetric(scanDiagnostics.serverDescriptorSpread),
        serverEmbeddingFrames: Number.isFinite(scanDiagnostics.serverEmbeddingFrames) ? Number(scanDiagnostics.serverEmbeddingFrames) : null,
        serverEmbeddingRejectedFrames: Number.isFinite(scanDiagnostics.serverEmbeddingRejectedFrames) ? Number(scanDiagnostics.serverEmbeddingRejectedFrames) : null,
        serverEmbeddingAverageMs: Number.isFinite(scanDiagnostics.serverEmbeddingAverageMs) ? Number(scanDiagnostics.serverEmbeddingAverageMs) : null,
      },
      performance,
      matchDebug: {
        ...
        bucketedCandidatesFound: Number.isFinite(matchDebug.bucketedCandidatesFound) ? Number(matchDebug.bucketedCandidatesFound) : null,
        fullCandidatesFound: Number.isFinite(matchDebug.fullCandidatesFound) ? Number(matchDebug.fullCandidatesFound) : null,
        bestDistance: roundMetric(matchDebug.bestDistance),
        secondDistance: roundMetric(matchDebug.secondDistance),
        secondViableDistance: roundMetric(matchDebug.secondViableDistance),
        ...
        queryWinCount: Number.isFinite(matchDebug.queryWinCount) ? Number(matchDebug.queryWinCount) : null,
        decisiveQueryWinCount: Number.isFinite(matchDebug.decisiveQueryWinCount) ? Number(matchDebug.decisiveQueryWinCount) : null,
        ...
        matchStrategy: String(matchDebug.matchStrategy || '').slice(0, 80),
