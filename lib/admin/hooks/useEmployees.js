...
export function useEmployees() {
  ...
  const handleEmployeeUpdate = useCallback(async (person, updates, successMsg = '') => {
    ...
    try {
      if (typeof updates?.lifecycleStatus === 'string') {
        await transitionPersonLifecycle(person, updates.lifecycleStatus, successMsg || 'Changed employee lifecycle from the employee directory.')
      } else {
  ...
  const handleBulkEmployeeUpdate = useCallback(async (
    ...
    updates,
    {
      ...
      reason = 'Changed employee lifecycle using a bulk directory action.',
    } = {},
  ) => {
    ...
    const worker = async () => {
      while (queue.length > 0) {
        ...
        try {
          if (typeof updates?.lifecycleStatus === 'string') {
            await transitionPersonLifecycle(person, updates.lifecycleStatus, reason)
          } else {
            await updatePersonRecord(person, updates)
          }
