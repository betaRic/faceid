...
export function buildDtrDocument({
  ...
  const signatory = (overrideName || overridePosition)
...
export function splitDtrEmployeeName(employee = {}) {
  const familyName = String(employee.familyName || employee.lastName || employee.surname || '').trim()
  const firstName = String(employee.firstName || employee.givenName || '').trim()
  const middleInitial = normalizeMiddleInitial(employee.middleInitial || employee.middleName || '')

  if (familyName || firstName || middleInitial) {
    return { familyName, firstName, middleInitial }
  }

  const rawName = String(employee.name || '').trim().replace(/\s+/g, ' ')
