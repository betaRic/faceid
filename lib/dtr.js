...
export function buildDtrDocument({
  ...
  for (let day = 1; day <= 31; day++) {
    ...
    const specialCode = isActive ? String(source?.specialCode || '').trim().toUpperCase() : ''
    ...
    rows.push({
      ...
      specialCode,
      specialLabel: isActive ? String(source?.specialLabel || specialCode).trim() : '',
      specialColor: isActive ? String(source?.specialColor || specialCode.toLowerCase()).trim() : '',
      scheduledWorking: source?.scheduledWorking !== false,
      scheduledTimes: source?.scheduledTimes || null,
      isAbsent: isActive && !specialCode && source?.scheduledWorking !== false && !hasAnyTime,
      amIn: isActive && !specialCode ? normalizeTimeValue(source?.amIn) : '',
      amOut: isActive && !specialCode ? normalizeTimeValue(source?.amOut) : '',
      pmIn: isActive && !specialCode ? normalizeTimeValue(source?.pmIn) : '',
      pmOut: isActive && !specialCode ? normalizeTimeValue(source?.pmOut) : '',
      undertime,
  ...
  return {
    ...
    officialHours: resolveDtrOfficialHours(office, rows),
    signatory,
    period: {
      month,
      year,
...
export function resolveDtrOfficialHours(office = null, rows = []) {
  const policy = office?.workPolicy || {}
  const scheduled = rows.filter(row => row.isActive && row.scheduledWorking && row.scheduledTimes).map(row => row.scheduledTimes)
  const primary = scheduled[0] || policy
  const sameSchedule = scheduled.every(item => ['morningIn', 'morningOut', 'afternoonIn', 'afternoonOut'].every(key => item[key] === primary[key]))
  const morningIn = normalizeClockLabel(primary.morningIn || '08:00')
  const morningOut = normalizeClockLabel(primary.morningOut || '12:00')
  const afternoonIn = normalizeClockLabel(primary.afternoonIn || '13:00')
  const afternoonOut = normalizeClockLabel(primary.afternoonOut || '17:00')

  return {
    regularDays: scheduled.length ? summarizeWorkingDays([...new Set(rows.filter(row => row.scheduledWorking).map(row => getDtrCalendarDay(Number(row.dateKey.slice(0, 4)), Number(row.dateKey.slice(5, 7)), row.day)))]) : summarizeWorkingDays(policy.workingDays),
    arrivalDeparture: sameSchedule ? `${morningIn}-${morningOut} to ${afternoonIn}-${afternoonOut}` : 'VARIES BY ASSIGNED DAILY SCHEDULE',
  }
}
