...
export async function getActiveThresholds(db) {
  ...
  try {
    if (postgresEnabled()) {
      const result = await queryPostgres('SELECT value FROM system_config WHERE key = $1 LIMIT 1', ['thresholds'])
      if (result.rows[0]) cache = sanitizeThresholds(result.rows[0].value)
      else cache = DEFAULTS
    } else {
...
export async function resetThresholdsToDefaults(db) {
  if (postgresEnabled()) {
    await queryPostgres('DELETE FROM system_config WHERE key = $1', ['thresholds'])
  } else {
