...
export const DEFAULTS = {
  ...
  minFaceAreaRatio: KIOSK_MIN_FACE_AREA_RATIO,
...
export async function setActiveThresholds(db, values) {
  const merged = { ...DEFAULTS, ...values, updatedAt: Date.now() }
...
export const THRESHOLD_META = {
  biometric: {
    ...
    fields: {
      ...
      ambiguousMargin: {
        ...
        description: 'Best match must beat 2nd best by this much. Set to 0 to disable ambiguity blocking entirely.',
        min: 0.0, max: 0.10, step: 0.01, default: AMBIGUOUS_MATCH_MARGIN,
        format: v => v.toFixed(2),
        zeroNote: 'Setting to 0 disables ambiguity blocking — use with caution.',
      },
  ...
  kiosk: {
    ...
    fields: {
      ...
      minFaceAreaRatio: {
        label: 'Min Face Area',
        description: 'Face must cover this fraction of the oval frame.',
        min: 0.02, max: 0.20, step: 0.01, default: KIOSK_MIN_FACE_AREA_RATIO,
