...
function normalizeRawAttendanceRow(row) {
  return {
    department: String(row?.divisionName || row?.department || row?.departmentName || 'Unassigned').trim() || 'Unassigned',
...
export function buildRawAttendanceWorksheets(attendanceRows = []) {
  ...
  for (const row of normalizedRows) {
    const key = `${row.department}::${row.employeeId || row.name}`
  ...
  const worksheets = Array.from(grouped.entries()).map(([key, rows]) => {
    const first = rows[0] || {}
    const sheetName = uniqueSheetName(`${first.department} - ${first.name || first.employeeId || key}`.trim(), usedNames)
    return {
      name: sheetName,
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  <dc:creator>VeriFace</dc:creator>
  <cp:lastModifiedBy>VeriFace</cp:lastModifiedBy>
...
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes">
  <Application>VeriFace</Application>
