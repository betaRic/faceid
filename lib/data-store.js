...
import { normalizeEmployeeNameFields } from './person-name'
import { PRIVACY_NOTICE_VERSION } from './privacy-consent'
import { normalizeEmployeeWfhDays } from './employee-wfh'

const serverApiEnabled = true
...
function mapPersonRecord(id, payload) {
  ...
  const names = normalizeEmployeeNameFields(payload)
  return {
    id,
    name: payload.name || names.name,
    ...
    individualWfhDays: normalizeEmployeeWfhDays(payload.individualWfhDays),
    active: payload.active !== false,
    approvalStatus: getEffectivePersonApprovalStatus(payload),
    sampleCount: descriptors.length,
    descriptors,
    duplicateReviewRequired: payload?.duplicateReviewRequired === true || ['required', 'review_required'].includes(payload?.duplicateReviewStatus),
...
function normalizeEmployeeId(employeeId) {
  return employeeId.trim().replace(/\D/g, '')
}

function stripRawAttendanceEvidence(entry = {}) {
  const {
    descriptor,
    descriptors,
    landmarks,
...
function stripClientBiometricsForServer(entry = {}) {
  const {
    descriptor,
    descriptors,
    landmarks,
...
function requireLocalBiometricFallback() {
  throw new Error(
    'Biometric enrollment and attendance require the local server API.',
  )
}
...
export async function upsertPersonSample(persons, profile, descriptors) {
  ...
  const normalizedSampleFrames = normalizeEnrollmentSampleFrames(profile?.sampleFrames)
  const descriptorValidationError = validateEnrollmentDescriptorBatch(normalizedDescriptors)
  const captureMetadataError = validateEnrollmentCaptureMetadata(
    profile.captureMetadata,
    serverApiEnabled ? normalizedSampleFrames : normalizedDescriptors,
  )
  const sampleFrameValidationError = validateEnrollmentSampleFrames(normalizedSampleFrames)
  const normalizedNames = normalizeEmployeeNameFields(profile)
  ...
  const divisionId = String(profile.divisionId || '').trim()

  if (captureMetadataError) throw new Error(captureMetadataError)
  if (serverApiEnabled && sampleFrameValidationError) throw new Error(sampleFrameValidationError)
  if (!serverApiEnabled && descriptorValidationError) throw new Error(descriptorValidationError)
  if (!normalizedNames.lastName) throw new Error('Last name is required')
  if (!normalizedNames.firstName) throw new Error('First name is required')
  if (!position) throw new Error('Position is required')
  if (profile?.privacyConsent !== true || profile?.privacyNoticeVersion !== PRIVACY_NOTICE_VERSION) {
    throw new Error('You must read and accept the Data Privacy Notice before submitting registration.')
  }

  const existing = normalizedEmployeeId
    ? persons.find(person => person.employeeId === normalizedEmployeeId)
    : null
  ...
  const response = await fetch('/api/persons', {
    ...
    body: JSON.stringify({
      ...
      sampleFrames: normalizedSampleFrames,
    }),
  })
  ...
  return {
    ...
    savedSampleCount: Number(payload?.savedSampleCount || normalizedSampleFrames.length || normalizedDescriptors.length),
...
export async function checkEnrollmentDuplicate(input, personId = '') {
  const normalizedDescriptors = normalizeEnrollmentDescriptorBatch(
...
export async function deactivatePersonRecord(persons, id) {
  if (!serverApiEnabled) {
    const nextPersons = persons.map(person => person.id === id
      ? { ...person, lifecycleStatus: 'inactive', approvalStatus: 'rejected', active: false }
      : person)
    saveLocalPersons(nextPersons)
  ...
  const payload = await response.json().catch(() => null)
  if (!response.ok) throw new Error(payload?.message || 'Failed to deactivate employee')
  return { mode: 'local', ...payload }
}

export async function updatePersonRecord(person, updates) {
  const names = normalizeEmployeeNameFields({ ...person, ...updates })
  const normalized = {
    name: names.name,
    lastName: names.lastName,
    firstName: names.firstName,
    middleName: names.middleName,
    employeeId: typeof updates.employeeId === 'string' ? updates.employeeId.trim().replace(/\D/g, '') : person.employeeId,
    position: typeof updates.position === 'string' ? updates.position.trim() : person.position,
    ...
    divisionId: typeof updates.divisionId === 'string' ? updates.divisionId : person.divisionId,
    divisionName: typeof updates.divisionName === 'string' ? updates.divisionName : person.divisionName,
    individualWfhDays: updates.individualWfhDays !== undefined
      ? normalizeEmployeeWfhDays(updates.individualWfhDays)
      : normalizeEmployeeWfhDays(person.individualWfhDays),
  }
...
export async function transitionPersonLifecycle(person, lifecycleStatus, reason) {
  const normalizedStatus = String(lifecycleStatus || '').trim().toLowerCase()
  const normalizedReason = String(reason || '').trim()
  if (!normalizedStatus) throw new Error('Lifecycle status is required')
  if (!normalizedReason) throw new Error('Lifecycle reason is required')
    ...
    const nextPersons = current.map(item => (
      item.id === person.id
        ? {
            ...
            approvalStatus: normalizedStatus === 'active' ? 'approved' : normalizedStatus === 'pending' ? 'pending' : 'rejected',
          }
        : item
    ))
    saveLocalPersons(nextPersons)
  ...
  const payload = await response.json().catch(() => null)
  if (!response.ok) throw new Error(payload?.message || 'Failed to change employee status')
  return { mode: 'local', person: payload?.person || null }
}
...
export async function logAttendanceEntry(entry) {
  ...
  const response = await fetch('/api/attendance/v2', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(stripClientBiometricsForServer(entry)),
  })
  ...
  if (!response.ok) {
    ...
    error.errorId = payload?.errorId || null
  ...
  return {
    ...
    fieldDutyPending: payload?.fieldDutyPending === true,
  }
}
...
export async function requestAttendanceChallenge(entry = {}) {
  ...
  const response = await fetch('/api/attendance/challenge', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(stripRawAttendanceEvidence(entry)),
  })
