...
export async function upsertPersonSample(persons, profile, descriptors) {
  ...
  if (descriptorValidationError) {
    throw new Error(descriptorValidationError)
  }
  ...
  const duplicateFace = normalizedDescriptors
    .map(descriptor => findDuplicateFace(persons, normalizedEmployeeId, descriptor))
    .find(Boolean)
