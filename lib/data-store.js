...
export async function upsertPersonSample(persons, profile, descriptors) {
  ...
  if (!firebaseEnabled) {
    ...
    // Local duplicate check (dev mode only)
    for (const descriptor of normalizedDescriptors) {
      const dup = findClosestPerson(persons, normalizedEmployeeId, descriptor, DUPLICATE_FACE_THRESHOLD)
      if (dup) {
        throw new Error(`Face is too similar to ${dup.person.name} (${dup.person.employeeId || 'no employee ID'})`)
      }
    }
...
export async function checkEnrollmentDuplicate(descriptors, personId = '') {
  ...
  if (!firebaseEnabled) {
    return { mode: 'local', duplicate: false, existingPerson: null, message: '' }
  }
...
export async function logAttendanceEntry(entry) {
  ...
  let payloadBody = entry

  if (!entry?.challenge?.token) {
    try {
      const challengeResponse = await fetch('/api/attendance/challenge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          kioskContext: entry?.kioskContext || null,
        }),
      })
      const challengePayload = await challengeResponse.json().catch(() => null)
      if (challengeResponse.ok && challengePayload?.challenge?.token) {
        endpoint = '/api/attendance/v2'
        payloadBody = {
          ...entry,
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn('[AttendanceAPI] Challenge bootstrap failed; falling back to legacy route.', challengeError?.message)
      }
    }
  } else {
    endpoint = '/api/attendance/v2'
  ...
  if (!response.ok) {
    ...
    error.personId = payload?.personId || null
    error.needsReenrollment = Boolean(payload?.needsReenrollment)
    error.reenrollmentReason = payload?.reenrollmentReason || null
    error.reenrollmentMessage = payload?.reenrollmentMessage || ''
    error.canSelfReenroll = Boolean(payload?.canSelfReenroll)
  ...
  return {
    ...
    needsReenrollment: Boolean(payload?.needsReenrollment),
    reenrollmentReason: payload?.reenrollmentReason || null,
    reenrollmentMessage: payload?.reenrollmentMessage || '',
    canSelfReenroll: Boolean(payload?.canSelfReenroll),
