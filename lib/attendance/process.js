...
import { getPersonLifecycleStatus, isPersonApproved } from '@/lib/person-approval'
...
import {
  ...
  buildAuthoritativeAttendancePayload as buildAuthoritativeAttendancePayloadDefault,
} from '@/lib/biometrics/server-attendance'
import { queueOpenVinoProfileUpdate } from '@/lib/biometrics/openvino-shadow-profile'
import { queueBiometricMemoryCollection } from '@/lib/biometrics/memory-collection'
import { normalizeEntry, validateClaimedEmployeeId, validateEntry } from './normalize'
import { checkAttendanceLocation } from './context'
import { getAttendanceLogsForDate, buildAttendanceEntryPreview } from './logs'
import { commitAcceptedAttendance, getCooldownForActionMinutes } from './write'
import { findClaimedEmployeeMatch as findClaimedEmployeeMatchDefault } from './match'
...
async function timeStep(timings, name, fn) {
  ...
  try {
    ...
    // The route owns the public error response. Preserve only the last safe
    // internal stage so its audit record can pinpoint an operational fault.
    if (error && typeof error === 'object' && !error.attendanceStage) {
      error.attendanceStage = name
    }
    throw error
  } finally {
...
function isFastSingleFrameEnabled() {
  return process.env.ATTENDANCE_FAST_SINGLE_FRAME_ENABLED === 'true'
}
const PRODUCTION_ATTENDANCE_SERVICES = Object.freeze({
  buildAuthoritativeAttendancePayload: buildAuthoritativeAttendancePayloadDefault,
  findClaimedEmployeeMatch: findClaimedEmployeeMatchDefault,
})

function resolveAttendanceServices(services) {
  return {
    buildAuthoritativeAttendancePayload: services?.buildAuthoritativeAttendancePayload
      || PRODUCTION_ATTENDANCE_SERVICES.buildAuthoritativeAttendancePayload,
    findClaimedEmployeeMatch: services?.findClaimedEmployeeMatch
      || PRODUCTION_ATTENDANCE_SERVICES.findClaimedEmployeeMatch,
  }
}
...
function applyAuthoritativePayload(entry, authoritativePayload, matchMode) {
  return {
    ...
    captureContext: {
      ...
      authoritativeAntispoofSource: authoritativePayload.diagnostics.modelVersion,
...
function buildRequestMeta(request, entry = {}) {
  return {
    ...
    employeeId: String(entry?.employeeId || '').trim().slice(0, 64),
  }
}
...
async function writeFailedScanLog(db, entry, decisionCode, reason, extra = {}) {
  try {
    ...
    const metadata = {
      ...
      employeeId: String(entry.employeeId || extra.employeeId || '').slice(0, 64),
...
function buildChallengeContext(entry, requestMeta, overrides = {}) {
  ...
  return {
    ...
    employeeId: requestMeta.employeeId || '',
...
export async function processAttendanceSubmission({ db, request, body, consumedChallenge = null, services = null }) {
  const {
    buildAuthoritativeAttendancePayload,
    findClaimedEmployeeMatch,
  } = resolveAttendanceServices(services)
  const timings = []
  ...
  const fastSingleFrameEnabled = isFastSingleFrameEnabled()
  ...
  const claimedEmployeeIdError = validateClaimedEmployeeId(entry.employeeId)
  if (claimedEmployeeIdError) {
    const decisionCode = entry.employeeId ? 'blocked_invalid_access_code' : 'blocked_missing_access_code'
    entry.riskFlags = mergeRiskFlags(entry.riskFlags, ['invalid_claimed_access_code'])
    await writeFailedScanLog(db, entry, decisionCode, claimedEmployeeIdError, withServerTimings({
      clientIp: requestMeta.clientIp,
      clientKey: requestMeta.clientKey,
    }, timings))
    return respond(
      { ok: false, message: claimedEmployeeIdError, decisionCode, riskFlags: entry.riskFlags },
      { status: 400 },
    )
  }

  let authoritativePayload = null
  let serverMatchMode = fastSingleFrameEnabled ? 'single_frame_fast' : 'two_frame_required'
  try {
    authoritativePayload = fastSingleFrameEnabled
    ...
    if (!fastSingleFrameEnabled) {
      const decisionCode = String(error.code || 'blocked_no_reliable_match')
    ...
    try {
      ...
      if (!(fallbackError instanceof SafeRequestError)) throw fallbackError
      const decisionCode = String(fallbackError.code || error.code || 'blocked_no_reliable_match')
  ...
  // added database latency without improving match accuracy.
  ...
  let personMatch = await timeStep(timings, serverMatchMode === 'single_frame_fast' ? 'match_1' : 'match_2', () => findClaimedEmployeeMatch(db, allOffices, entry.descriptor, {
    entry,
    geofenceContext,
  }))

  if (
    fastSingleFrameEnabled
    &&
    ...
    try {
      ...
      personMatch = await timeStep(timings, 'match_2', () => findClaimedEmployeeMatch(db, allOffices, entry.descriptor, {
        entry,
        geofenceContext,
      }))
    } catch (error) {
  ...
  const matchedPersonId = String(person?.id || personMatch.personId || '').trim()
  if (!matchedPersonId) {
    await writeFailedScanLog(db, entry, 'blocked_no_reliable_match', 'Matched employee has no canonical person ID.', withServerTimings({
      clientIp: requestMeta.clientIp,
      clientKey: requestMeta.clientKey,
    }, timings))
    return respond(
      { ok: false, message: 'Matched employee record is invalid.', decisionCode: 'blocked_no_reliable_match' },
      { status: 403 },
    )
  }
  entry = {
  ...
  if (getPersonLifecycleStatus(person) === 'pending') {
  ...
  if (person.active === false || !isPersonApproved(person)) {
  ...
  const dailyLogs = await timeStep(timings, 'daily_logs', () => getAttendanceLogsForDate(db, entry.employeeId, entry.dateKey, legacyDateLabel, entry.personId))
  let policyOverride
  try {
    policyOverride = await timeStep(timings, 'workforce_policy', () => resolveWorkforcePolicyForDate({ person, office, dateKey: entry.dateKey }))
  } catch (error) {
    // Work-time policy changes the legal attendance calculation, so fail
    // closed rather than recording an entry with an invented schedule.
    console.error('[attendance] Workforce policy unavailable', { internalMessage: error?.message || String(error) })
    const message = 'Attendance schedule is temporarily unavailable. Contact HR or an administrator.'
    await writeFailedScanLog(db, entry, 'blocked_workforce_policy_unavailable', message, withServerTimings({
      employeeId: person.employeeId,
      officeId: person.officeId,
      clientIp: requestMeta.clientIp,
      clientKey: requestMeta.clientKey,
    }, timings))
    return respond({ ok: false, message, decisionCode: 'blocked_workforce_policy_unavailable' }, { status: 503 })
  }
  const nextAction = getNextAttendanceAction(dailyLogs, office, entry.timestamp, policyOverride)

  if (nextAction === 'complete') {
    const latestDailyEntry = dailyLogs.length > 0 ? buildAttendanceEntryPreview(dailyLogs[dailyLogs.length - 1]) : null
    const employeeViewPayload = await timeStep(timings, 'employee_session', () => buildEmployeeViewSessionPayload(db, person, entry.personId))
    return attachEmployeeViewSessionCookie(respond({ ok: false, message: 'Full day attendance already recorded.', decisionCode: 'blocked_day_complete', entry: latestDailyEntry, ...employeeViewPayload }, { status: 409 }), employeeViewPayload)
  }

  let locationResult = checkAttendanceLocation(person, office, entry, allOffices)
  const fieldDutyRequested = entry.fieldDuty?.requested === true
    && Boolean(entry.fieldDuty?.reason)
    && Boolean(entry.fieldDuty?.remarks)
  if (!locationResult.ok && fieldDutyRequested && locationResult.decisionCode === 'blocked_geofence') {
    locationResult = {
      ok: true,
      attendanceMode: 'Field Duty',
      geofenceStatus: 'Outside office radius — pending HR/Admin approval',
      decisionCode: 'pending_field_duty',
    }
    entry.fieldDutyStatus = 'pending'
    entry.fieldDutyReason = entry.fieldDuty.reason
    entry.fieldDutyRemarks = entry.fieldDuty.remarks
    entry.source = 'field_duty'
    entry.requestedAction = nextAction
    entry.fieldDutyRequestedAt = new Date().toISOString()
  }
  ...
  const writeResult = await timeStep(timings, 'write_attendance', () => commitAcceptedAttendance({
    entry,
    person,
    office,
    policyOverride,
  ...
  if (!writeResult.ok) {
    if (writeResult.reason === 'complete') {
      const employeeViewPayload = await timeStep(timings, 'employee_session', () => buildEmployeeViewSessionPayload(db, person, entry.personId))
      return attachEmployeeViewSessionCookie(respond({
        ok: false,
        message: 'Full day attendance already recorded.',
        decisionCode: 'blocked_day_complete',
        entry: writeResult.entry,
        ...employeeViewPayload,
      }, { status: 409 }), employeeViewPayload)
    }

    const lockedAction = writeResult.action
    const blockedEntry = {
      ...entry,
      action: lockedAction,
      ...(entry.fieldDutyStatus === 'pending' ? { requestedAction: lockedAction } : {}),
    }
    const cooldownMinutes = getCooldownForActionMinutes(office, lockedAction)
    const employeeViewPayload = await timeStep(timings, 'employee_session', () => buildEmployeeViewSessionPayload(db, person, entry.personId))
    await writeFailedScanLog(db, blockedEntry, 'blocked_recent_duplicate', `Duplicate ${lockedAction} attempt within cooldown window`, withServerTimings({
      employeeId: person.employeeId,
      officeId: person.officeId,
      clientIp: requestMeta.clientIp,
      clientKey: requestMeta.clientKey,
    ...
    const response = respond(
      {
        ok: false,
        message: `${lockedAction === 'checkin' ? 'Check-in' : 'Check-out'} available again after ${cooldownMinutes} minute(s).`,
  ...
  entry.action = writeResult.action
  if (entry.fieldDutyStatus === 'pending') entry.requestedAction = writeResult.action
  queueBiometricMemoryCollection({ person, entry, attendanceId: writeResult.attendanceId, personMatch, authoritativePayload })
  ...
  const responsePayload = {
    ...
    ...(await timeStep(timings, 'employee_session', () => buildEmployeeViewSessionPayload(db, person, entry.personId))),
  }
  if (process.env.NODE_ENV !== 'production') {
    ...
    responsePayload.debug = debug
      ? {
          ...
          matchMode: debug.matchMode || debug.matchStrategy || null,
          claimedEmployeeId: debug.claimedEmployeeId || null,
          resolvedEmployeeId: debug.resolvedEmployeeId || null,
          serverMatchMode,
        }
      : null
  }
