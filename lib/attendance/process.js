...
export async function processAttendanceSubmission({ db, request, body, consumedChallenge = null }) {
  ...
  let serverMatchMode = 'single_frame_fast'
  try {
    authoritativePayload = await timeStep(timings, 'server_embed_1', () => buildAuthoritativeAttendancePayload(entry.scanFrames, {
      frameLimit: 1,
      minFrames: 1,
    }))
    ...
    try {
      ...
      await writeFailedScanLog(db, entry, decisionCode, message, {
        clientIp: requestMeta.clientIp,
        clientKey: requestMeta.clientKey,
      })
      return respond(
  ...
  if (
    ...
    && Array.isArray(entry.scanFrames)
    && entry.scanFrames.length >= ATTENDANCE_MIN_SERVER_FRAMES
  ) {
    try {
      serverMatchMode = 'two_frame_fallback'
      authoritativePayload = await timeStep(timings, 'server_embed_2', () => buildAuthoritativeAttendancePayload(entry.scanFrames, {
      ...
      if (!fullCapturePolicy.ok) {
        await writeFailedScanLog(db, entry, fullCapturePolicy.decisionCode, fullCapturePolicy.message, {
          clientIp: requestMeta.clientIp,
          clientKey: requestMeta.clientKey,
        })
        return respond(
