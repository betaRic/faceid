...
import { buildAuthoritativeAttendancePayload } from '@/lib/biometrics/server-attendance'
...
async function countRecentBlockedScans(db, requestMeta) {
  ...
  try {
    ...
    return snapshot.docs.reduce((count, record) => (
      (record.data() || {}).status === 'blocked' ? count + 1 : count
    ), 0)
  } catch {
    return 0
  }
...
export async function processAttendanceSubmission({ db, request, body, consumedChallenge = null }) {
  ...
  try {
    const authoritativePayload = await buildAuthoritativeAttendancePayload(entry.scanFrames)
    entry = {
      ...entry,
