...
export function resolveChallengeMode(riskFlags = []) {
  const needsActive = riskFlags.some(flag => (
    flag === 'outside_office_geofence'
    || flag === 'pad_gray_zone'
    || flag === 'low_strict_frames'
    || flag === 'low_descriptor_spread'
    || flag === 'recent_failures'
    || flag === 'landscape_mobile_capture'
  ))

  return needsActive ? 'active' : 'passive'
}
