...
function normalizeScanFrames(value, maxCount = 2) {
  return Array.isArray(value)
    ? value
        .slice(0, maxCount)
        .map(frame => {
          if (typeof frame === 'string') return { frameDataUrl: String(frame || '') }
...
export function normalizeEntry(body) {
  ...
  const fieldDuty = body?.fieldDuty && typeof body.fieldDuty === 'object' ? body.fieldDuty : {}
  return {
    ...
    fieldDuty: {
      requested: fieldDuty.requested === true,
      reason: String(fieldDuty.reason || '').trim().slice(0, 80),
      remarks: String(fieldDuty.remarks || '').trim().slice(0, 500),
    },
    ...
    scanFrames: normalizeScanFrames(body?.scanFrames),
    ...
    livenessEvidence: livenessEvidence ? {
      ...
      meshDeltas: Array.isArray(livenessEvidence.meshDeltas)
        ...
        : [],
      irisDeltas: Array.isArray(livenessEvidence.irisDeltas)
        ? livenessEvidence.irisDeltas.slice(0, 30).map(v => (v === null ? null : Number(v)))
        : [],
      ...
      avgIrisDelta: livenessEvidence.avgIrisDelta == null ? null : Number(livenessEvidence.avgIrisDelta),
      ...
      hasEyeSignal: Boolean(livenessEvidence.hasEyeSignal),
      hasMotionSignal: Boolean(livenessEvidence.hasMotionSignal),
...
export function validateClaimedEmployeeId(employeeId) {
  const value = String(employeeId || '').trim()
  if (!value) return 'VeriFace access code is required before scanning.'
  if (!/^\d{4}$/.test(value)) return 'VeriFace access code must be exactly four digits.'
  return null
}
