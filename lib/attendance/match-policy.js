...
export const MATCH_SUPPORT_STRONG_SINGLE_DISTANCE = 0.60
export const MATCH_SUPPORT_STRONG_GLOBAL_MARGIN = 0.18
...
export function isStrongUnambiguousSingleSampleSupport(support, matchDebug = {}) {
  if (!support?.weakSingleSample) return false

  const bestDistance = Number(support.bestDistance)
  const secondDistance = Number(matchDebug?.secondDistance)
  ...
  return (
    Number.isFinite(bestDistance)
    && bestDistance <= MATCH_SUPPORT_STRONG_SINGLE_DISTANCE
    && Number.isFinite(margin)
    && margin >= MATCH_SUPPORT_STRONG_GLOBAL_MARGIN
    && Number(support.supportCount || 0) >= 1
  )
}
