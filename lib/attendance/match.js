import { AMBIGUOUS_MATCH_MARGIN, DISTANCE_THRESHOLD, DISTANCE_THRESHOLD_KIOSK, DISTANCE_THRESHOLD_ENROLLMENT } from '@/lib/config'
import {
...
export async function findMatchFromCandidates(db, candidateOfficeIds, descriptor, isKioskVerification = true) {
  ...
  if (indexedCandidates.length > 0) {
    const indexedMatch = await resolveMatchedPerson(
      db,
      matchBiometricIndexCandidates(indexedCandidates, descriptor, threshold, AMBIGUOUS_MATCH_MARGIN),
    )
    if (indexedMatch.ok) {
      return indexedMatch
    }
