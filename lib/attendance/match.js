...
function buildEmployeeIdLookupVariants(employeeId) {
...
async function resolvePersonByEmployeeId(db, employeeId) {
  const lookupVariants = buildEmployeeIdLookupVariants(employeeId)
  if (lookupVariants.length === 0) {
    return {
      ok: false,
      decisionCode: 'blocked_missing_employee_id',
      message: 'Employee ID is required before scanning.',
      debug: {
        source: 'claimed_employee_1to1',
        matchMode: 'claimed_employee_1to1',
      },
    }
  }

  if (postgresEnabled()) {
    const person = await getLocalPersonByEmployeeId(employeeId)
    if (!person) {
      return {
        ok: false,
        decisionCode: 'blocked_unknown_employee_id',
        message: 'Employee ID was not found. Check the ID and try again.',
        debug: {
          source: 'claimed_employee_1to1',
          matchMode: 'claimed_employee_1to1',
          claimedEmployeeId: String(employeeId || '').trim(),
          lookupVariants,
        },
      }
    }
  ...
  snapshots.forEach(snapshot => {
    snapshot?.docs?.forEach(record => {
      if (!records.has(record.id)) {
        records.set(record.id, { id: record.id, ...record.data() })
      }
    })
  })
  ...
  if (records.size > 1) {
    return {
      ok: false,
      decisionCode: 'blocked_employee_id_conflict',
      message: 'Employee ID matches multiple records. Ask HR to fix the employee directory.',
      debug: {
        source: 'claimed_employee_1to1',
        matchMode: 'claimed_employee_1to1',
        claimedEmployeeId: String(employeeId || '').trim(),
...
export async function findClaimedEmployeeMatch(db, allOffices, descriptor, options = {}) {
  ...
  const claimedEmployeeId = String(options.employeeId || entry.employeeId || '').trim()
  const lookupResult = await resolvePersonByEmployeeId(db, claimedEmployeeId)
  ...
  const debugBase = {
    ...
    officeId: String(person?.officeId || ''),
    lookupVariants: lookupResult.lookupVariants || [],
  }
  ...
  if (!matchResult.ok) {
    return {
      ...
      message: matchResult.decisionCode === 'blocked_ambiguous_match'
        ? matchResult.message
        : 'Face does not match the entered employee ID.',
      debug: {
        ...(matchResult.debug || {}),
        ...debugBase,
