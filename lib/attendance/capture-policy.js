...
function hasStrongTemporalLiveness(validation) {
  return Boolean(
    validation?.ok
    && validation.hasEyeSignal
    && validation.hasMotionSignal
  )
}
...
export function getScanCapturePolicyAssessment(entry) {
  ...
  const strongTemporalLiveness = hasStrongTemporalLiveness(livenessValidation)

  if (Number(entry.antispoof) <= PAD_ANTISPOOF_HARD_BLOCK_THRESHOLD) {
    if (!strongTemporalLiveness) {
      return {
        ok: false,
        decisionCode: 'blocked_antispoof',
