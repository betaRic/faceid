...
function hasUsableTemporalLiveness(validation) {
  return Boolean(
    validation?.ok
    && (validation.hasEyeSignal || validation.hasMotionSignal)
  )
}
...
export function getScanCapturePolicyAssessment(entry) {
  ...
  const usableTemporalLiveness = hasUsableTemporalLiveness(livenessValidation)

  if (Number(entry.antispoof) <= PAD_ANTISPOOF_HARD_BLOCK_THRESHOLD) {
    if (!usableTemporalLiveness) {
      return {
        ok: false,
        decisionCode: 'blocked_antispoof',
    ...
    riskFlags.push('single_frame_pad_low')
  }
  ...
  if (Number(entry.liveness) <= PAD_LIVENESS_HARD_BLOCK_THRESHOLD) {
    ...
    riskFlags.push('single_frame_liveness_low')
  }
