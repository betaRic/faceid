...
export const SCAN_CAPTURE_POLICY_VERSION = 'scan-v3'
...
export function getScanCapturePolicyAssessment(entry) {
  ...
  if (livenessValidation && !livenessValidation.ok) {
    if (livenessValidation.reason === 'static_image_detected') {
      return {
        ok: false,
        decisionCode: 'blocked_liveness',
    ...
    if (livenessValidation.reason === 'antispoof_hard_fail') {
      return {
        ok: false,
        decisionCode: 'blocked_antispoof',
    ...
    riskFlags.push('weak_temporal_liveness')
  }
