...
function SummaryPanelInner() {
  ...
  function handleOverrideSaved() {
    // The SummaryPanel already refetches on date/filter change, so bump the
    // date to force a reload without changing it visually.
              ...
              {summaryRows.length === 0 ? (
                ...
                summaryRows.map(row => (
                  <tr key={`${row.employeeId}-${row.dateKey}`} className="bg-white">
                    ...
                    <td className="px-5 py-3">
                      <button
                        className="rounded-full border border-black/10 bg-white px-3 py-1.5 text-xs font-semibold text-ink transition hover:bg-stone-100"
