...
export default function EmployeeEditorModal({ person, onSave, onCancel }) {
  ...
  return (
    <section className="h-full min-h-0 bg-surface p-3 text-foreground sm:p-6">
      <Surface className="flex h-full min-h-0 w-full flex-col overflow-hidden">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            {accessCode ? <p className="mt-1 text-xs font-semibold text-primary">VeriFace access code: {accessCode}</p> : null}
            {submittedLabel && currentLifecycle === PERSON_LIFECYCLE_PENDING && (
              <p className="mt-1 text-xs text-warning">{submittedLabel}</p>
            )}
          </div>
          <IconButton
        ...
        {photoUrl ? (
          <div className="mt-5 overflow-hidden rounded-2xl border border-line bg-stone-950">
            <img
          ...
          <div className="mt-5 rounded-2xl border border-warning-line bg-warning-surface px-4 py-3 text-sm text-warning">
        ...
        <div className="mt-4 rounded-xl border border-line bg-subdued px-3 py-3 text-sm text-secondary">
          ...
          {phaseSampleCounts ? (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {phaseLabels.map(([phaseId, label]) => (
                <span key={phaseId} className="rounded-full bg-surface px-2 py-1 text-[11px] font-semibold text-secondary">
        ...
        {currentLifecycle === PERSON_LIFECYCLE_PENDING && (
          <div className="mt-5 rounded-2xl border-2 border-warning-line bg-warning-surface p-4">
            <h3 className="text-sm font-semibold text-warning">Pending review</h3>
            <p className="mt-1 text-sm text-warning">Review this enrollment before activating the employee.</p>
            {person.duplicateReviewRequired ? (
              <div className="mt-3 rounded-xl border border-destructive-line bg-destructive-surface px-3 py-3 text-sm text-destructive">
            ...
            <div className="mt-4 flex gap-3">
              <Button
                className="flex-1 border-success bg-success text-success-contrast hover:brightness-110"
                disabled={isPending(`employee-approve-${person.id}`)}
                onClick={handleQuickApprove}
              >
                {isPending(`employee-approve-${person.id}`) ? <><span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-success-contrast border-t-transparent" />Activating...</> : 'Activate'}
              </Button>
              <Button
                className="flex-1"
        ...
        {currentLifecycle === PERSON_LIFECYCLE_ACTIVE && (
          <div className="mt-5">
            <div className="mb-4 flex items-center justify-between rounded-xl border border-line bg-subdued p-4">
              <div>
            ...
            <div className="grid gap-4">
              ...
              <Field label="Office">
                <select className="w-full rounded-xl border border-line bg-surface px-3 py-2 text-sm text-foreground outline-none transition focus:border-primary" onChange={(e) => { setOfficeId(e.target.value); setDivisionId('') }} value={officeId}>
              ...
              <fieldset className="grid gap-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[['Mon', 1], ['Tue', 2], ['Wed', 3], ['Thu', 4], ['Fri', 5], ['Sat', 6], ['Sun', 0]].map(([label, day]) => {
                    ...
                    return <button aria-pressed={activeDay} className={`min-h-11 rounded-control border px-3 py-2 text-sm font-semibold transition ${activeDay ? 'border-warning-line bg-warning-surface text-warning' : 'border-line bg-surface text-secondary hover:bg-canvas'}`} key={day} onClick={() => toggleIndividualWfhDay(day)} type="button">{label}</button>
                  })}
                </div>
              </fieldset>

              <div className="rounded-xl border border-line bg-subdued p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-primary">Employee schedule override</p>
                  ...
                  <div className="mt-3 space-y-2">
                    {[['Mon', 1], ['Tue', 2], ['Wed', 3], ['Thu', 4], ['Fri', 5], ['Sat', 6], ['Sun', 0]].map(([label, day]) => {
                      ...
                      return <div className="grid gap-2 rounded-control border border-line bg-surface p-3 text-xs md:grid-cols-[34px_58px_44px_1fr_1fr] md:items-center md:border-0 md:bg-transparent md:p-0" key={day}>
                        ...
                        <div className="grid grid-cols-2 gap-1"><input aria-label={`${label} AM in`} className="min-w-0 rounded border border-line bg-surface px-1 py-1.5 text-foreground disabled:bg-subdued" disabled={!overridden || entry.working === false} onChange={(event) => updateScheduleDay(day, 'morningIn', event.target.value)} type="time" value={entry.morningIn || '08:00'} /><input aria-label={`${label} AM out`} className="min-w-0 rounded border border-line bg-surface px-1 py-1.5 text-foreground disabled:bg-subdued" disabled={!overridden || entry.working === false} onChange={(event) => updateScheduleDay(day, 'morningOut', event.target.value)} type="time" value={entry.morningOut || '12:00'} /></div>
                        <div className="grid grid-cols-2 gap-1"><input aria-label={`${label} PM in`} className="min-w-0 rounded border border-line bg-surface px-1 py-1.5 text-foreground disabled:bg-subdued" disabled={!overridden || entry.working === false} onChange={(event) => updateScheduleDay(day, 'afternoonIn', event.target.value)} type="time" value={entry.afternoonIn || '13:00'} /><input aria-label={`${label} PM out`} className="min-w-0 rounded border border-line bg-surface px-1 py-1.5 text-foreground disabled:bg-subdued" disabled={!overridden || entry.working === false} onChange={(event) => updateScheduleDay(day, 'afternoonOut', event.target.value)} type="time" value={entry.afternoonOut || '17:00'} /></div>
                      </div>
                    })}
                  </div>
                ) : null}
                <div className="mt-3 border-t border-line pt-3">
                  ...
                  {flexitimeEnabled ? <label className="mt-2 flex items-center gap-2 text-xs text-secondary">Required minutes per work day <input className="w-20 rounded border border-line bg-surface px-2 py-1 text-foreground" min="1" onChange={(event) => setFlexitimeRequiredMinutes(Number(event.target.value) || 480)} type="number" value={flexitimeRequiredMinutes} /></label> : null}
                </div>
              </div>

              <div className="rounded-xl border border-line bg-subdued px-3 py-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-primary">VeriFace access code</p>
              ...
              <div className="rounded-xl border border-line bg-subdued px-3 py-2 text-sm text-secondary">
                {(person.sampleCount ?? 0) > 0
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  ...
                  onClick={handleSave}
                >
                  {isSaving ? <><span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-primary-contrast border-t-transparent" />Saving...</> : 'Save changes'}
                </Button>
              </div>
            </div>
          </div>
        ...
        {(currentLifecycle === PERSON_LIFECYCLE_INACTIVE || currentLifecycle === PERSON_LIFECYCLE_REJECTED) && (
          <div className={`mt-5 rounded-2xl border p-4 ${currentLifecycle === PERSON_LIFECYCLE_REJECTED ? 'border-destructive-line bg-destructive-surface' : 'border-line bg-subdued'}`}>
            <p className={`text-sm font-medium ${currentLifecycle === PERSON_LIFECYCLE_REJECTED ? 'text-destructive' : 'text-secondary'}`}>
